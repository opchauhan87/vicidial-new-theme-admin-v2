	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			if (strlen($only_field) < 1)
				...
				$CFoutput .= "<center><div class='col-sm-12 bg-gray' style='overflow:auto;height:64vh;'><div class='row'><TABLE cellspacing=2 cellpadding=2>\n";
			...
			while ($fields_to_print > $o) 
				...
				if ( ($A_field_type[$o]!='SCRIPT') and ($A_field_type[$o]!='HIDDEN') and ($A_field_type[$o]!='HIDEBLOB') and (strlen($only_field) < 1) )
						{$CFoutput .= "<div class='form-group col-md-3'><label class='custom-form-label'><B>$A_field_name[$o]</B></label>";}
				...
				//end
					...
					$field_HTML .= "<select class='form-control' size=1 name=$A_field_label[$o] id=$A_field_label[$o] $change_trigger>\n";
					}
				if ($A_field_type[$o]=='MULTI')
					{
					$field_HTML .= "<select class='form-control' MULTIPLE size=$A_field_size[$o] name=$A_field_label[$o][] id=$A_field_label[$o][]>\n";
					}
					//chirag
					//16-03-21
...
//end
				...
				if ($A_field_type[$o]=='TEXT') 
					...
					$field_HTML .= "<input class= 'form-control customer-page' type=text size=$A_field_size[$o] maxlength=$A_field_max[$o] name=$A_field_label[$o] id=$A_field_label[$o] value=\"$A_field_value[$o]\" $change_trigger>\n";
					if ( ($A_field_required[$o] == 'Y') or ( ($A_field_required[$o] == 'INBOUND_ONLY') and (preg_match("/^Y\d\d\d\d\d\d\d/",$call_id)) ) )
				...
				if ($A_field_type[$o]=='AREA') 
					...
					if ($A_field_default[$o]=='NULL') {$A_field_default[$o]='';}
					if (strlen($A_field_value[$o]) < 1) {$A_field_value[$o] = $A_field_default[$o];}
					$field_HTML .= "<textarea class='form-control' name=$A_field_label[$o] id=$A_field_label[$o] ROWS=$A_field_max[$o] COLS=$A_field_size[$o] $change_trigger>$A_field_value[$o]</textarea>";
					if ( ($A_field_required[$o] == 'Y') or ( ($A_field_required[$o] == 'INBOUND_ONLY') and (preg_match("/^Y\d\d\d\d\d\d\d/",$call_id)) ) )
						{$custom_required_fields .= "$A_field_label[$o]|";}
					}
