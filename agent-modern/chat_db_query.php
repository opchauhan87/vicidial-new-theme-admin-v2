...
if ($action=="ReloadAgentNewChatSpan" && $user) {
	echo "<table style='width:100%;' width='600' border='0' cellpadding='5' cellspacing='0'>\n";
	...
	$loop_count=0;
	echo "<select style='height: 40px; width: 50%; border-radius: 10px; margin: 10px; border-color: #b6ccd7; padding: 5px;' name='agent' id='agent'>\n";
	...
	echo "</td>\n";
	echo "<td width='200' style='width:40%;'><font class='arial'>"._QXZ("Message").":</font><BR>\n";
	echo "<textarea style='margin-top:10px; width: 93%; height: 40vh; padding: 10px; border-radius: 10px;' class='small_arial' rows='5' cols='36' name='agent_message' id='agent_message'></textarea>";
	echo "</td></TR>\n";

	echo "<TR BGCOLOR='#E6E6E6'>\n";
	echo "<td><BR><input class='iframe-btn-red' type='button' style='width:200px' value='"._QXZ("BACK TO CHAT SCREEN")."' onClick=\"document.getElementById('AgentChatSpan').style.display='block'; document.getElementById('AgentNewChatSpan').style.display='none';\"></td>\n";
	echo "<td align='center'><BR><input class='iframe-btn-green' type='button' style='width:200px' value='"._QXZ("START CHAT")."' onClick=\"CreateAgentToAgentChat()\">\n</td></TR>\n";
	echo "</table>";
...
if ($action=="xfer_chat" && $user && $chat_id && $chat_xfer_value && $chat_xfer_type) {
	...
	if (mysqli_affected_rows($link)==1) {
		...
		echo "<BR/><BR/><input class='iframe-btn-green' type='button' style=\"width:150px\" value=\""._QXZ("START CHAT")."\" onClick=\"StartChat()\">";
