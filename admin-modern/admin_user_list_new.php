...
echo "<div id=\"pcont4\" class=\"container-fluid\">
			  ...
			  <div class=\"block-flat\">
			  <div class=\"header\">
			  <h3>"._QXZ("User List New Limits Form")." :</h3>
			  </div>";

echo "<center><div class=\"col-md-12\"><div class=\"content\">\n";
echo "<tr><td class='text-left' align=center colspan=2>\n";
$FORM_list_id='';
$FORM_user='';
if ( ($stage == 'overall') and ($list_id == 'NONE') )
	{
	echo "<b>"._QXZ("User Overall New Limit Entries ALL USERS",0,'')."</b> &nbsp; $NWB#user_list_new_limits$NWE\n";
	}
else
	{
	if ($list_id != '---ALL---') 
...
echo "<TABLE class='table_list hover centerline' width=740 cellspacing=3>\n";
if ($stage == 'overall')
	{echo "<thead><tr><th>#</th><th>"._QXZ("USER")."</th><th>"._QXZ("NEW LIMIT")."</th><th>"._QXZ("TODAY")."</th></tr></thead>\n";}
else
	{echo "<thead><tr><th>#</th><th>"._QXZ("USER")."</th><th>"._QXZ("LIST")."</th><th>"._QXZ("ACTIVE")."</th><th>"._QXZ("CAMPAIGN")."</th><th>"._QXZ("NEW LIMIT")."</th><th>"._QXZ("TODAY")."</th></tr></thead>\n";}
...
###############################################################################
	...
	if ($users_to_print > 0) 
		...
		while ($lists_to_print > $o) 
			...
			echo "<tr $bgcolor>";
			echo "<td>$ct</td>";
			echo "<td><a href=\"admin.php?ADD=3&user=$Ruser\">$Ruser</a> - $Rfull_name</td>";
			echo "<td><a href=\"admin.php?ADD=311&list_id=$Rlist_id[$o]\">$Rlist_id[$o]</a> - $Rlist_name[$o]</td>";
			echo "<td>$Ractive[$o]</td>";
			echo "<td><a href=\"admin.php?ADD=34&campaign_id=$Rcampaign[$o]\">$Rcampaign[$o]</a></td>";
			echo "<td><input class='form-control' type=text size=4 maxlength=4 name=$Ruser$US$Rlist_id[$o] value=\"$Ruser_override\"></td>";
			echo "<td>$Rnew_count</td>";
			echo "</tr>\n";
...
if ( ($list_id != '---ALL---') and ($list_id != 'NONE') )
	...
	if ($lists_to_print > 0) 
		...
		while ($users_to_print > $o) 
			...
			echo "<tr $bgcolor>";
			echo "<td>$ct</td>";
			echo "<td><a href=\"admin.php?ADD=3&user=$Ruser[$o]\">$Ruser[$o]</a> - $Rfull_name[$o]</td>";
			echo "<td><a href=\"admin.php?ADD=311&list_id=$Rlist_id\">$Rlist_id</a> - $Rlist_name</td>";
			echo "<td>$Ractive</td>";
			echo "<td><a href=\"admin.php?ADD=34&campaign_id=$Rcampaign\">$Rcampaign</a></td>";
			echo "<td><input class='form-control' type=text size=4 maxlength=4 name=$Ruser[$o]$US$Rlist_id value=\"$Ruser_override\"></td>";
			echo "<td>$Rnew_count</td>";
			echo "</tr>\n";
...
echo "<td colspan=4 class=text-center><input class='btn btn-bdr-sucess' type=submit name=SUBMIT value='"._QXZ("SUBMIT")."'></td>";
echo "</tr>\n";
...
if ( ($LOGuser_level >= 9) and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL REPORTS/",$LOGallowed_reports)) ) )
	{
	echo "<center><a class='btn btn-primary btn-sm' href=\"$changesLINK\">"._QXZ("Click here to see Admin changes to this record")."</a></center>\n";
	}
...
?>
</div></div></TABLE></div></div></div></div></div></center>
</body>
</html>
