...
$MAIN.='<div id="pcont4" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3 class="pull-left"> '._QXZ("$report_name").'</h3>';
		
$MAIN.="<div class='pull-right'><a class='btn btn-info' href='AST_LAGGED_log_report_summary.php'>"._QXZ("SUMMARY")."</a></div>	";	
		
$MAIN.='</div><div class="col-md-12"><div class="content">';
//$MAIN.="<TABLE CELLPADDING=4 CELLSPACING=0><TR><TD>";
$MAIN.="<FORM ACTION=\"$PHP_SELF\" METHOD=GET name=vicidial_report id=vicidial_report>\n";
$MAIN.="<TABLE class='table1 col_table' BORDER=0 cellspacing=5 cellpadding=5 class='col-lg-12 spacer'><TR><TD VALIGN=TOP align=center class='no-border col-md-6 spacer'>\n";
$MAIN.="<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">";
//~ $MAIN.="<div class='col-lg-2 size'>"._QXZ("Date").":</div>";
$MAIN.="<div class='col-lg-10'><div class=\"form-group custom-input mb-10\"><INPUT TYPE=TEXT NAME=query_date SIZE=10 class='form-control' MAXLENGTH=10 VALUE=\"$query_date\"><label>"._QXZ("Date")."</label></div>";
$MAIN.="</div><div class='col-md-1 no-padding'><script language=\"JavaScript\">\n";
...
$MAIN.="<div class='col-lg-3 no-padding' style='margin-left: 15px;'><INPUT TYPE=TEXT class='form-control' NAME=query_date_D SIZE=9 MAXLENGTH=8 VALUE=\"$query_date_D\"></div>";

$MAIN.="<div class='col-lg-1 no-padding text-center'> "._QXZ("to")."</div><div class='col-lg-3 no-padding'> <INPUT TYPE=TEXT NAME=query_date_T SIZE=9 class='form-control' MAXLENGTH=8 VALUE=\"$query_date_T\"></div>";

$MAIN.="</TD><TD VALIGN=TOP align=center class='no-border col-md-6 spacer'>\n";
//~ $MAIN.="<div class='col-lg-3 size'>"._QXZ("Display as").":</div>";
$MAIN.="<div class='col-lg-4'><div class=\"form-group custom-input mb-10\"><select name='report_display_type' class='form-control'>";
if ($report_display_type) {$MAIN.="<option value='$report_display_type' selected>$report_display_type</option>";}
$MAIN.="<option value='TEXT'>"._QXZ("TEXT")."</option><option value='HTML'>"._QXZ("HTML")."</option></select><label>"._QXZ("Display as")."</label></div></TD>";
$MAIN.="</TD></TR><TR><TD VALIGN=TOP align=center class='no-border' colspan=3 style='text-align:center;'><INPUT TYPE=submit class='btn btn-bdr-sucess' NAME=SUBMIT VALUE='"._QXZ("SUBMIT")."'>\n";
$MAIN.="</TD></TR></TABLE>\n";
if ($SUBMIT && $query_date) {
	...
	if (mysqli_num_rows($rslt)>0) {
		...
		$ASCII_text.="+-----------------+---------+\n";
		$HTML_text.="<label>"._QXZ("SERVER IP BREAKDOWN FOR LAGGED RECORDS")." $query_date, $query_date_D "._QXZ("TO")." $query_date_T</label>";
		$HTML_text.="<table class='table_list hover centerline' border='0' cellpadding='0' cellspacing='2' width='350'>";
		$HTML_text.="<thead><TR><TH class=''>"._QXZ("SERVER IP")."</th><TH class=''>"._QXZ("COUNT")."</th></tr></thead>";
		...
		while ($row=mysqli_fetch_array($rslt)) {
			...
			$ASCII_text.="|\n";
			$HTML_text.="<TR><TD>$row[server_ip]</td><TD class=''>$row[ct]</td></tr>";
			$total_count+=$row["ct"];
		}
		$ASCII_text.="+-----------------+---------+\n";
		...
		$HTML_rpt.="<label>"._QXZ("LAGGED LOG RECORDS FOR")." $query_date, $query_date_D "._QXZ("TO")." $query_date_T $server_rpt_string, "._QXZ("RECORDS")." #$lower_limit-$upper_limit<a href=\"$PHP_SELF?SUBMIT=$SUBMIT&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">["._QXZ("DOWNLOAD")."]</a></label>";
		$HTML_text.="<table border='0' class='table_list hover centerline' cellpadding='0' cellspacing='2' width='1000'>";
		$HTML_rpt.="<thead><TR><TH>"._QXZ("AGENT LOG ID")."</TH><TH>"._QXZ("USER")."</TH><TH>"._QXZ("SERVER IP")."</TH><TH>"._QXZ("EVENT TIME")."</TH><TH>"._QXZ("LEAD ID")."</TH><TH>"._QXZ("CAMPAIGN")."</TH><TH>"._QXZ("STATUS")."</TH><TH>"._QXZ("USER GROUP")."</TH><TH>"._QXZ("COMMENTS")."</TH><TH>"._QXZ("UNIQUE ID")."</TH></TR></thead>";
		...
		for ($i=1; $i<=mysqli_num_rows($rpt_rslt); $i++) {
			...
			if ($i>=$lower_limit && $i<=$upper_limit) {
				...
				$HTML_rpt.="<TR valign='top'><td>$row[agent_log_id]</td><td>$row[user]</td><td>$row[server_ip]</td><td>$row[event_time]</td><td>$row[lead_id]</td><td>$row[campaign_id]</td><td>$row[status]</td><td>$row[user_group]</td><td>$row[comments]</td><td>$row[uniqueid]</td></TR>";
