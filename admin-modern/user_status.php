...
<div id="pcont4" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3 class='pull-left'> <?php echo _QXZ("User Status for"); ?> <?php echo $user ?> </h3><div class="pull-right"><a class="btn btn-info btn-sm" href="./user_stats.php?user=<?php $user ?>"><?php echo _QXZ("User Stats") ?></a><a class="btn btn-info btn-sm" href="./admin.php?ADD=3&user=<?php echo $user ?>"><?php echo _QXZ("Modify User") ?></a><a class="btn btn-info btn-sm" href="./AST_agent_days_detail.php?user=<?php echo $user ?>&query_date=<?php echo $REPORTdate ?>&end_date=<?php echo $REPORTdate ?>&group[]=--ALL--&shift=ALL"><?php echo _QXZ("User multiple day status detail report") ?></a><a class="btn btn-info btn-sm" href="./AST_agent_time_sheet.php?agent= <?php $user ?>"><?php echo _QXZ("Agent Time Sheet") ?></a></div></div>
		<div class="col-md-12"><div class="content">
<TABLE WIDTH=<?php echo $page_width ?> BGCOLOR=#E6E6E6 cellpadding=2 cellspacing=0 class='no-border'>

<?php 

echo "<TR class='no-border'><TD ALIGN=LEFT COLSPAN=2 class='no-border' style='text-align: left;'>\n";
...
if ($agents_to_print > 0)
	...
	echo "<TABLE class='hover' CELLPADDING=0 CELLSPACING=0>";
	...
	if ($change_agent_campaign > 0)
		...
		echo "<div class='col-lg-2 spacer no-padding'>"._QXZ("Current Campaign").":</div> <div class='col-lg-3'><SELECT class='form-control' SIZE=1 NAME=group>\n";
		$o=0;
		...
		echo "</SELECT></div>\n";
		echo "<input  style='background-color:#$SSbutton_color' class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("CHANGE")."' disabled><BR></form>\n";

		echo "<form action=$PHP_SELF method=POST>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=user value=\"$user\">\n";
		echo "<input type=hidden name=stage value=\"log_agent_out\">\n";
		echo "<input  style='background-color:#$SSbutton_color' class='btn btn-bdr-sucess' type=submit name=submit value=\""._QXZ("EMERGENCY LOG AGENT OUT")."\"><BR></form>\n";
		}
	}

else
	{
	echo '<div class="col-lg-12 size spacer">'._QXZ("Agent is not logged in")."</div>\n<BR>";
	}
...
if ( ($Tstatus == "LOGIN") or ($Tstatus == "START") )
	{
	echo '<div class="col-lg-12 size spacer">'._QXZ("User")." $user($full_name) - "._QXZ("is logged in to the timeclock").". <BR>"._QXZ("Login time").": $Tevent_date "._QXZ("from")." $Tip_address</div><BR>\n";
	$TC_log_change_stage =	'tc_log_user_OUT';
	$TC_log_change_button = _QXZ("TIMECLOCK LOG THIS USER OUT");
	}
else
	{
	echo '<div class="col-lg-12 size spacer">'._QXZ("User")." $user($full_name) - "._QXZ("is NOT logged in to the timeclock").".</div><BR><div class=\"col-lg-12 size spacer\">"._QXZ("Last logout time").": $Tevent_date "._QXZ("from")." $Tip_address</div><BR>\n";
...
if ($modify_timeclock_log > 0)
	...
	echo "<div class=\"col-lg-12 spacer\"><input style='background-color:#$SSbutton_color' type=submit name=submit value=\"$TC_log_change_button\" class='btn btn-bdr-sucess'></div><BR></form>\n";
	echo "<BR><BR>\n";
	}
...
echo "</B></TD></TR>\n";
echo "<TR><TD style='border-top:none ;' ALIGN=LEFT COLSPAN=2>\n";
