...
if ($qc_action=="new_scorecard") 
	...
	echo "\t<tr nowrap style='background: #31708f;
    color: #fff;' class='add_new'>\n";
	echo "\t<th colspan='2'><font color='white'>"._QXZ("ADD NEW SCORECARD")."</font></th>\n";
	echo "</tr>\n";
	echo "<tr>\n";
	echo "\t<td align='right' style='width: 20%;'>"._QXZ("Scorecard ID").":</td>\n";
	echo "\t<td align='left' class='d-flex'><input type='text' size='6' class='form-control' maxlength='8' name='new_scorecard_id' class='form-control'>$NWB#qc_scorecards-new_scorecard_id$NWE</td>\n";
	echo "</tr>\n";
	echo "<tr>\n";
	echo "\t<td align='right' style='width: 20%;'>"._QXZ("Scorecard name").":</td>\n";
	echo "\t<td align='left' class='d-flex'><input type='text' size='20' maxlength='255' class='form-control' name='new_scorecard_name'>$NWB#qc_scorecards-new_scorecard_name$NWE</td>\n";
	echo "</tr>\n";
	echo "<tr>\n";
	echo "\t<td align='right'  style='width: 20%;'>"._QXZ("Active").":</td>\n";
	echo "\t<td align='left' class='d-flex'><select name='new_active' class='form-control'><option value='Y'>"._QXZ("Y")."</option><option value='N'>"._QXZ("N")."</option></select>$NWB#qc_scorecards-new_active$NWE</td>\n";
	echo "</tr>\n";
	echo "\t<tr nowrap>\n";
	echo "\t<th colspan='2'><input type='submit' name='submit' class='btn alert-success' value='"._QXZ("SUBMIT NEW SCORECARD")."'></th>\n";
	echo "</tr>\n";
	echo "</table>\n";
	echo "</form>\n";
	}
...
if ($scorecard_id && $qc_action=="load_checkpoints") {
	...
	if (mysqli_num_rows($rslt)>0) {
		//~ echo "<tr>";
		echo "<div class='header'><h3 style='text-align:left;'>"._QXZ("MODIFY QC SCORECARD")." $scorecard_id $NWB#qc_scorecards-modify$NWE</div>";
		//~ echo "</tr>";
		//~ echo "<tr>";
		echo "<div class='col-md-12'><div class='col-md-3' style='text-align: right;'><label>"._QXZ("NAME").": </label></div><div class='col-md-9' style='text-align: left;margin-top: -10px;margin-bottom: 5px;'><input type='text' size='20' maxlength='255' class='form-control' id='scorecard_name".$scorecard_id."' name='scorecard_name".$scorecard_id." onBlur=\"ChangeCheckpoint('0', '$scorecard_id', 'update_scorecard', 'scorecard_name')\" value='$scorecard_name'></div></div>";
		//~ echo "</tr>";
		$i++;
		if ($i%2==0) {$bgcolor=$SSstd_row1_background;} else {$bgcolor=$SSstd_row2_background;}
		echo "<thead><tr>\n";
		echo "<th class='' nowrap>"._QXZ("Order")."$NWB#qc_scorecards-order$NWE</th>";
		echo "<th class='' nowrap>"._QXZ("Active")."$NWB#qc_scorecards-active_checkpoint$NWE</th>";
		echo "<th class='' nowrap>"._QXZ("Checkpoint Text")."$NWB#qc_scorecards-checkpoint_text$NWE</th>";
		echo "<th class='' nowrap>"._QXZ("Preset comments")."$NWB#qc_scorecards-preset_comments$NWE</th>";
		echo "<th class='' nowrap>"._QXZ("Points")."$NWB#qc_scorecards-points$NWE</th>";
		echo "<th class='' nowrap>"._QXZ("Instant Fail")."$NWB#qc_scorecards-instant_fail$NWE</th>";
		echo "<th class='' nowrap>"._QXZ("Admin Notes")."$NWB#qc_scorecards-admin_notes$NWE</th>";
		...
		while ($row=mysqli_fetch_array($rslt)) {
			...
			echo "<select class='form-control' onChange=\"ChangeCheckpoint('".$row["checkpoint_row_id"]."', '$scorecard_id', 'update_checkpoint', 'checkpoint_rank', '$row[checkpoint_rank]', this.value)\" name='checkpoint_rank".$row["checkpoint_row_id"]."' id='checkpoint_rank".$row["checkpoint_row_id"]."'>";
			...
			echo "<td align='center'>";
			echo "<textarea class='form-control' rows='4' cols='40' onBlur=\"ChangeCheckpoint('".$row["checkpoint_row_id"]."', '$scorecard_id', 'update_checkpoint', 'checkpoint_text')\" id='checkpoint_text".$row["checkpoint_row_id"]."' name='checkpoint_text".$row["checkpoint_row_id"]."'>".$row["checkpoint_text"]."</textarea>";
			echo "</td>\n";
			echo "<td align='center'>";
			echo "<textarea class='form-control' rows='4' cols='20' onBlur=\"ChangeCheckpoint('".$row["checkpoint_row_id"]."', '$scorecard_id', 'update_checkpoint', 'checkpoint_text_presets')\" id='checkpoint_text_presets".$row["checkpoint_row_id"]."' name='checkpoint_text_presets".$row["checkpoint_row_id"]."'>".$row["checkpoint_text_presets"]."</textarea>";
			echo "</td>\n";

			echo "<td align='center'><input class='form-control' type='text' onBlur=\"ChangeCheckpoint('".$row["checkpoint_row_id"]."', '$scorecard_id', 'update_checkpoint', 'checkpoint_points')\" id='checkpoint_points".$row["checkpoint_row_id"]."' name='checkpoint_points".$row["checkpoint_row_id"]."' value='".$row["checkpoint_points"]."' size='2' maxlength='3'></td>\n";
			echo "<td align='center'>";
			...
			echo "</td>";
			echo "<td align='center'><textarea class='form-control' rows='2' cols='25' onBlur=\"ChangeCheckpoint('".$row["checkpoint_row_id"]."', '$scorecard_id', 'update_checkpoint', 'admin_notes')\" id='admin_notes".$row["checkpoint_row_id"]."' name='admin_notes".$row["checkpoint_row_id"]."'>".$row["admin_notes"]."</textarea></td>\n";
			# echo "<td align='center'>";
			...
			# echo "</td>";
			echo "<td align='center'><input type='button' class='btn btn-danger' onClick=\"ChangeCheckpoint('".$row["checkpoint_row_id"]."', '$scorecard_id', 'delete_checkpoint')\" value='"._QXZ("DELETE")."'></td>";
			echo "</tr>";
		}
		echo "<tr><td colspan='3' class='small_display_header' align='right'>"._QXZ("PASSING SCORE").":</td><td align='center'><input type='text' size='2' maxlength='3' class='form-control' id='passing_score".$scorecard_id."' name='passing_score".$scorecard_id."' onBlur=\"ChangeCheckpoint('0', '$scorecard_id', 'update_scorecard', 'passing_score')\" value='$passing_score'></td><td align='left' class='small_display_header'>/ $max_score</td><td colspan='3'>&nbsp;</td></tr>";
	} else {
		echo "<tr><th colspan='8' class=''>** "._QXZ("CURRENTLY NO CHECKPOINTS FOR SCORECARD")." $scorecard_id **</td></tr>\n";
		echo "<tr>";
		echo "<th colspan='8' class=''><label>"._QXZ("NAME")."</label>: <input type='text' class='form-control' size='20' maxlength='255' id='scorecard_name".$scorecard_id."' name='scorecard_name".$scorecard_id."' onBlur=\"ChangeCheckpoint('0', '$scorecard_id', 'update_scorecard', 'scorecard_name')\" value='$scorecard_name'></th>";
		echo "</tr>";
	}
	...
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	//~ echo "<tr>";
	echo "<div class='header'><h3 style='text-align:left;'>"._QXZ("ADD NEW CHECKPOINT FOR SCORECARD")." $scorecard_id</h3></div>";
	//~ echo "</tr>";
	echo "<tr><thead>";
	echo "<th class='' nowrap>"._QXZ("Order")."$NWB#qc_scorecards-order$NWE</th>";
	echo "<th class='' nowrap>"._QXZ("Active")."$NWB#qc_scorecards-active_checkpoint$NWE</th>";
	...
	echo "<tr>";
	echo "<td align='center'>";
	echo "<select class='form-control' name='new_checkpoint_rank' id='new_checkpoint_rank'>";
	for ($i=1; $i<=$next_rank; $i++) {
	...
	echo "</td>";
	echo "<td align='center'>";
	echo "<textarea class='form-control' rows=\"4\" cols=\"40\" id=\"new_checkpoint_text\" name=\"new_checkpoint_text\">**"._QXZ("Enter display text here")."**</textarea>";
	echo "</td>\n";
	echo "<td align='center'>";
	echo "<textarea class='form-control' rows=\"4\" cols=\"20\" id=\"new_checkpoint_text_presets\" name=\"new_checkpoint_text_presets\"></textarea>";
	echo "</td>\n";
	echo "<td align='center'>";
	echo "<input type='text' name='new_checkpoint_points' class='form-control' id='new_checkpoint_points' size='2' maxlength='3' value='0'>";
	echo "</td>";
	echo "<td align='center'>";
	echo "<input type='checkbox' name='new_instant_fail' id='new_instant_fail' value='Y'>";
	echo "</td>";
	echo "<td align='center'><textarea class='form-control' rows=\"2\" cols=\"25\" id=\"new_admin_notes\" name=\"new_admin_notes\"></textarea></td>\n";
#	echo "<td align='center'>";
...
if ($qc_action=="reload_scorecard_display") 
	{
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	echo "\t<thead><tr style=' background: #31708f;
    color: #fff;'>\n";
	echo "\t\t<th align=left><font size=1 color=white>"._QXZ("Scorecard")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("Checkpoints")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("Pass/max score")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("Campaigns")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("In-groups")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("Lists")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("Last modified")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>"._QXZ("Active")."</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>&nbsp;</font></td>\n";
	echo "\t\t<th align=left><font size=1 color='white'>&nbsp;</font></td>\n";
	echo "\t</tr></thead>\n";
   ...
   ' >\n";
		...
		$qc_scorecard_id= $row["qc_scorecard_id"];
		echo "\t\t\t<td align=left><input type='button' class='modify_bg_img' onClick=\"LoadCheckpoints('$row[qc_scorecard_id]')\" value=''></td>\n";
		echo "\t\t\t<td align=left><input type='button' class='delete_bg_img' onClick=\"window.location.href='$PHP_SELF?scorecard_id=$qc_scorecard_id&action=delete_scorecard'\" value=''></font></td>\n";
		echo "</tr>";
		}
	echo "\t<tr bgcolor=black>\n";
	echo "\t\t<th colspan='10' style='background: #fff;text-align:center;'><font size=1 color='white'><input style='margin: 10px 10px !important;' type='button' class='btn btn-primary' onClick='NewScorecard()' value='"._QXZ("ADD NEW SCORECARD")."'></font></td>\n";
	echo "\t</tr>\n";
	echo "</table>\n";
	}
