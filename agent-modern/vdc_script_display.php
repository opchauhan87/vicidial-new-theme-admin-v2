...
$stmt = "SELECT use_non_latin,timeclock_end_of_day,agentonly_callback_campaign_lock,custom_fields_enabled,enable_languages,language_method,enable_second_script,allow_web_debug FROM system_settings;";
#if ($DB) {echo "$stmt\n";}
	...
	if (count($iframe_matches)>0)
		...
		for ($q=0; $q<count($iframe_replace); $q++) 
			...
			if (preg_match($current_attribute, $iframe_replace[$q], $src_matches))
				...
				$src_match = preg_replace('/--A--user--B--/i',"$user",$src_match);
				$src_match = preg_replace('/--A--pass--B--/i',"$pass",$src_match);
...
if ( ( ($IFRAME < 1) and ($ScrollDIV > 0) ) or (preg_match("/IGNORENOSCROLL/i",$script_text)) )
	...
	//~ end
	echo "<div class=\"\" style='overflow:auto;' id=\"NewScriptContents\">";
	}
else
	{
	//~ ishwari
