...
echo '<div id="pcont4" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3> '._QXZ("USERONLY Callback Transfer").'</h3> </div>
		<div class="col-md-12"><div class="content">';
?>


<TABLE style=''  cellpadding=2 cellspacing=0>
<?php 
...
if ($SUBMIT && $old_user && $new_user && !$confirm_transfer) 
	...
	echo "<a class='btn btn-info' href='$PHP_SELF?DB=$DB&old_user=$old_user&new_user=$new_user&confirm_transfer=1&SUBMIT=1'>"._QXZ("CLICK TO CONFIRM")."</a><BR><BR>";
	echo "<a class='btn btn-info' href='$PHP_SELF?DB=$DB'>"._QXZ("CLICK TO CANCEL")."</a><BR><BR>";
	} 
else 
	{
	...
	if ($DB) {echo "$stmt\n";}
	# <tr><td align='right'>
	echo "<div class='col-md-9'><b>"._QXZ("Agents with callbacks").": </b><select class='form-control col-md-3' name='old_user' size=5>\n";
	...
	echo "</select></div><br>";
		echo "<div class='col-md-9 text-center' style='margin-top: 10px;
    margin-bottom: 10px;'><input type='submit' name='SUBMIT' value='  "._QXZ("TRANSFER TO")." ' class='btn btn-bdr-sucess'></div>";

	$stmt="select user, full_name from vicidial_users $ul_clause $LOGadmin_viewable_groupsSQL order by user asc";
	if ($DB) {echo "$stmt\n";}
	$rslt=mysql_to_mysqli($stmt, $link);
	echo "<div class='col-md-9'><select class='form-control' name='new_user' size=5>\n";
