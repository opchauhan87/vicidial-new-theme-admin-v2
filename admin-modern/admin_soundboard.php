...
echo "<div class='container-fluid' id='pcont4'>
		...
		<div class='page-head'>
		<a href='admin_soundboard.php?ADD=162000000000'> Show Soundboard Entries </a>
			|   
		<a href='admin_soundboard.php?ADD=162111111111'>Add A New Soundboard Entry </a>
		|   
		<a href='admin_soundboard.php?ADD=162211111111'>Copy A Soundboard Entry </a>
		</div></div></div></div>";
...
if ( (!preg_match("/soundboard/i",$SSactive_modules)) and ($SSagent_soundboards < 1) )
	{
	echo "<span class='message_block'><B><font color=red>"._QXZ("ERROR: Soundboards are not active on this system")."</B></font></span>\n";
	exit;
	}
...
if ($ADD==162111111111)
	...
	if ( ($modify_audiostore==1) and ($LOGmodify_scripts==1) )
		...
		//echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			  ...
			  <div class=\"block-flat\">
			  <div class=\"header\">
			  <h3>"._QXZ("ADD NEW SOUNDBOARD ENTRY")."</h3>
			  </div>";
		...
		echo "<center><div class=\"col-md-12\"><div class=\"content\"><TABLE class='table1 col_table' width=$section_width cellspacing=3>\n";

			echo  "<tr ><td align=right>"._QXZ("Soundboard ID").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=soundboard_id size=50 maxlength=100></div>$NWB#soundboard-soundboard_id$NWE</td></tr>\n";
		echo "<tr ><td align=right>"._QXZ("Soundboard Name").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=avatar_name size=70 maxlength=255></div>$NWB#soundboard-avatar_name$NWE</td></tr>\n";
		echo "<tr ><td align=right>"._QXZ("Soundboard Notes").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=avatar_notes size=70 maxlength=1000></div>$NWB#soundboard-avatar_name$NWE</td></tr>\n";
		echo "<tr ><td align=right>"._QXZ("Soundboard API User").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=avatar_api_user size=20 maxlength=20></div>$NWB#soundboard-avatar_api_user$NWE</td></tr>\n";
		echo "<tr ><td align=right>"._QXZ("Soundboard API Pass").": </td><td align=left><div class='col-lg-5'><input  class='form-control' type=password name=avatar_api_pass size=40 maxlength=100></div>$NWB#soundboard-avatar_api_pass$NWE</td></tr>\n";
		...
		echo "</select></div>$NWB#soundboard-user_group$NWE</td></tr>\n";

		echo "<tr ><td align=center colspan=2><p style='text-align:center'><input class=\"btn btn-bdr-sucess\" type=submit name=submit VALUE='"._QXZ("SUBMIT")."'></p></td></tr>\n";
	...
	if ( ($modify_audiostore==1) and ($LOGmodify_scripts==1) )
		...
		//echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<div id=\"pcont4\" class=\"container-fluid\">
			  ...
			  <div class=\"block-flat\">
			  <div class=\"header\">
			  <h3>"._QXZ("COPY SOUNDBOARD ENTRY")."</h3>
			  </div>";
		...
		echo "<tr ><td align=right>"._QXZ("Source Soundboard ID").": </td><td align=left><div class=\"col-lg-5\"><select class=\"form-control\"size=1 name=source_avatar_id>\n";
		...
		echo "$groups_list";
		echo "</select></div>$NWB#soundboard-soundboard_id$NWE</td></tr>\n";
...
if ($ADD==262111111111)
	...
	if ($add_copy_disabled > 0)
		...
		$row=mysqli_fetch_row($rslt);
		if ($row[0] > 0)
			{echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY NOT ADDED - there is already an soundboard entry in the system with this ID")."</span>\n";}
		else
			{
			if ( (strlen($soundboard_id) < 2) or (strlen($avatar_name) < 3) )
				{echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY NOT ADDED - Please go back and look at the data you entered")."</span>\n";}
			else
				{
				...
				$row=mysqli_fetch_row($rslt);
				if ($row[0] < 1)
					{echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY NOT ADDED - invalid API user")."</span>\n";}
				else
					{
					echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY ADDED")."</span>\n";
	...
	if ($add_copy_disabled > 0)
		...
		if ($row[0] > 0)
			...
			if ( (strlen($soundboard_id) < 2) or (strlen($avatar_name) < 3) or (strlen($source_avatar_id) < 2) )
				...
				$row=mysqli_fetch_row($rslt);
				if ($row[0] < 1)
					{echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY NOT COPIED - invalid source soundboard")."</span>\n";}
				else
					{
					echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY COPIED")."</span>\n";
	...
	if ( ($modify_audiostore==1) and ($LOGmodify_scripts==1) )
		...
		if ($stage == "FILEDELETE")
			...
			if ( (strlen($soundboard_id) < 2) or (strlen($audio_filename) < 1) or (strlen($rank) < 1) )
				{echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY NOT MODIFIED - Please go back and look at the data you entered")."</span>\n";}
			else
				{
				...
				echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY MODIFIED").": $soundboard_id - $audio_filename</span>\n";
			...
			if ( (strlen($soundboard_id) < 2) or (strlen($avatar_name) < 3) )
				...
				echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY MODIFIED").": $soundboard_id</span>\n";
...
if ($ADD==562111111111)
	...
	if (strlen($soundboard_id) < 2)
		{
		echo "<span class='message_block'>"._QXZ("SOUNDBOARD ENTRY NOT DELETED - Please go back and look at the data you entered")."\n";
		echo "<br>"._QXZ("SOUNDBOARD ID be at least 2 characters in length")."</span>\n";
		}
	else
		{
		echo "<span class='message_block'><B>"._QXZ("SOUNDBOARD ENTRY DELETION CONFIRMATION").": $soundboard_id - $avatar_name</B>\n";
		echo "<br><br><a href=\"$PHP_SELF?ADD=662111111111&soundboard_id=$soundboard_id&DB=$DB&ConFiRm=YES\">"._QXZ("Click here to delete soundboard entry")." $soundboard_id - $avatar_name</a></span>\n";
		}
...
if ($ADD==662111111111)
	...
	if ( (strlen($soundboard_id) < 2) or ($ConFiRm != 'YES') )
		...
		echo "<span class='message_block'><B>"._QXZ("SOUNDBOARD DELETION COMPLETED").": $soundboard_id</B></span>\n";
		//~ echo "<br><br>\n";
		}
	...
	//	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		...
		$columns_limit =	$row[10];
			...
			echo "<div id=\"pcont4\" class=\"container-fluid\">
			  ...
			  <div class=\"col-md-12 no-padding\">
			  <div class=\"block-flat\">
			  <div class=\"header\"><h3>"._QXZ("MODIFY A SOUNDBOARDS RECORD").": $soundboard_id </h3>
			  </div>";
		...
		echo "<input type=hidden name=soundboard_id value=\"$soundboard_id\">\n";
			...
			echo "<div class=\"col-md-12\"><div class=\"content\"><TABLE class='hover'>";
		echo "<tr ><td align=right>"._QXZ("Soundboard ID").": </td><td align=left><B>$soundboard_id</B></td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Soundboard Name").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=avatar_name size=70 maxlength=100 value=\"$avatar_name\"></div>$NWB#soundboard-avatar_name$NWE</td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Soundboard Notes").": </td><td align=left><div class='col-lg-5'><input class='form-control'type=text name=avatar_notes size=70 maxlength=1000 value=\"$avatar_notes\"></div>$NWB#soundboard-avatar_notes$NWE</td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Soundboard API User").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=avatar_api_user size=20 maxlength=20 value=\"$avatar_api_user\"></div>$NWB#soundboard-avatar_api_user$NWE</td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Soundboard API Pass").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=password name=avatar_api_pass size=40 maxlength=100 value=\"$avatar_api_pass\"></div>$NWB#soundboard-avatar_api_pass$NWE</td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Active").": </td><td align=left><div class='col-lg-5'><select class='form-control' size=1 name=active><option value='N'>"._QXZ("N")."</option><option value='Y'>"._QXZ("Y")."</option><option value='$active' SELECTED>"._QXZ("$active")."</option></select></div>$NWB#soundboard-active$NWE</td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Audio Functions").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=audio_functions size=70 maxlength=100 value=\"$audio_functions\"></div>$NWB#soundboard-audio_functions$NWE</td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Audio Display").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=audio_display size=70 maxlength=100 value=\"$audio_display\"></div>$NWB#soundboard-audio_display$NWE</td></tr>\n";
		
		echo "<tr><td align=right>"._QXZ("Layout").": </td><td align=left><div class='col-lg-5'><select class='form-control' size=1 name=soundboard_layout><option value='default'>"._QXZ("default")."</option><option value='columns01'>"._QXZ("columns01")."</option><option value='$soundboard_layout' SELECTED>"._QXZ("$soundboard_layout")."</option></select></div>$NWB#soundboard-soundboard_layout$NWE &nbsp; "._QXZ("Columns Limit").": <input type=text name=columns_limit size=3 maxlength=2 value=\"$columns_limit\"></td></tr>\n";
		...
		echo "<tr><td class=text-center colspan=2><lable><b>"._QXZ("Audio Files").": </b></lable></td></tr>\n";
		...
		while ($soundboardfiles_to_print > $o)
			...
			$ro = ($o + 1);
			$rowx=mysqli_fetch_row($rsltx);
			echo "<tr ><td><B>$ro. $rowx[0]</B> &nbsp; </td>";
			echo "<td ><div><select class='form-control' size=1 name=\"$rowx[0]--$rowx[3]--$rowx[2]--\">";
			...
			echo "</select></div></td>\n";
			echo "<td ><div><select  class='form-control' size=1 name=\"--HORD--$rowx[0]--$rowx[3]--$rowx[2]--\">";
			...
			echo "</select></div></td>\n";
			echo "<td ><div><select class='form-control' size=1 name=\"--TYPE--$rowx[0]--$rowx[3]--$rowx[2]--\">";
			...
			echo "</select></div></td>\n";
			echo "<td><div><select class='form-control' size=1 name=\"--FONT--$rowx[0]--$rowx[3]--$rowx[2]--\">";
			...
			echo "</select></div></td>\n";
			echo "<td ><font size=2> $rowx[3] </td>";
			echo "<td ><div class='col-lg-5'><input class='form-control' type=hidden name=\"--LEVEL--$rowx[0]--$rowx[3]--$rowx[2]--\" id=\"--LEVEL--$rowx[0]--$rowx[3]--$rowx[2]--\" value=\"$rowx[3]\">";
			...
			echo "<td nowrap> "._QXZ("Name").": <input type=text class=\"form-control\" name=\"--NAME--$rowx[0]--$rowx[3]--$rowx[2]--\" size=20 maxlength=100 value=\"$rowx[1]\"></td>";

			echo "<td nowrap> - <a class=\"btn btn-bdr-pink\" href=\"$PHP_SELF?ADD=462111111111&soundboard_id=$soundboard_id&stage=FILEDELETE&audio_filename=$rowx[0]&rank=$rowx[2]&level=$rowx[3]\">"._QXZ("DELETE")."</a></td></tr>\n";
			...
			while ($Csoundboardfiles_to_print > $Co)
				...
				echo "<td nowrap><font size=2> &nbsp; &nbsp; </font><select class='form-control' size=1 name=\"$rowC[0]--$rowC[3]--$rowC[2]--$rowx[0]\">\n";
				...
				echo "</select></td>\n";
				echo "<td nowrap><font size=2> &nbsp; &nbsp; </font><select class='form-control' size=1 name=\"--HORD--$rowC[0]--$rowC[3]--$rowC[2]--$rowx[0]\">\n";
				...
				echo "</select></td>\n";
				echo "<td nowrap><select class='form-control' size=1 name=\"--TYPE--$rowC[0]--$rowC[3]--$rowC[2]--$rowx[0]\">";
				echo "<option SELECTED value='$rowC[7]'>"._QXZ("$rowC[7]")."</option>\n";
				echo "<option value='button'>"._QXZ("button")."</option>\n";
				echo "<option value='header'>"._QXZ("header")."</option>\n";
				echo "<option value='space'>"._QXZ("space")."</option>\n";
				echo "</select></td>\n";
				echo "<td nowrap><select class='form-control' size=1 name=\"--FONT--$rowC[0]--$rowC[3]--$rowC[2]--$rowx[0]\">";
				...
				echo " &nbsp; </td>";
				echo "<td nowrap> &nbsp; "._QXZ("Name").": <input class='form-control' type=text name=\"--NAME--$rowC[0]--$rowC[3]--$rowC[2]--$rowx[0]\" size=20 maxlength=100 value=\"$rowC[1]\"></td>\n";

				echo "<td nowrap> &nbsp; - <a class=\"btn btn-bdr-pink\" href=\"$PHP_SELF?ADD=462111111111&soundboard_id=$soundboard_id&stage=FILEDELETE&audio_filename=$rowC[0]&rank=$rowC[2]&level=$rowC[3]&parent_filename=$rowx[0]\">"._QXZ("DELETE")."</a></td></tr>\n";
				$Co++;
				}
			$o++;
			}
		...
		echo "<tr><td nowrap><B>"._QXZ("NEW")."</B> &nbsp; </td>";
		...
		echo "<tr><td align=right>"._QXZ("Add An Audio File").": </td><td><div class='col-lg-5'><input class='form-control' type=text size=70 maxlength=100 name=audio_filename id=audio_filename value=\"\"> </div><a href=\"javascript:launch_chooser('audio_filename','date',600);\">"._QXZ("audio chooser")."</a>  $NWB#soundboard-audio_filename$NWE</td></tr>\n";

		echo "<tr ><td class=text-center colspan=2><input class='btn btn-bdr-sucess' type=submit name=submit VALUE='"._QXZ("SUBMIT")."'></td></tr>\n";
		echo "</TABLE></center>\n";
		...
		if ($modify_audiostore > 0)
			{
			echo "<br><br><a class='btn btn-danger btn-sm' href=\"$PHP_SELF?ADD=562111111111&soundboard_id=$soundboard_id&DB=$DB&avatar_name=$avatar_name\">"._QXZ("DELETE SOUNDBOARD ENTRY")."</a>\n";
			}
		if ( ($LOGuser_level >= 9) and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL REPORTS/",$LOGallowed_reports)) ) )
			{
			echo "<a class='btn btn-primary btn-sm' href=\"admin.php?ADD=720000000000000&category=SOUNDBOARDS&stage=$soundboard_id\">"._QXZ("Click here to see Admin changes to this Soundboard entry")."</a>\n";
			}
		}
	else
		{
...
if ($ADD==162000000000)
   ...
   <div class=\"row\">
   <div class=\"col-md-12 no-padding\">
   <div class=\"block-flat\"><div class=\"header\"><h3 class='pull-left'>"._QXZ("AGENT AUDIO SOUNDBOARD LISTINGS")." </h3><div class='pull-right'>";
	echo "</div></div><center><div class=\"col-md-12\"><div class=\"content\">
	 <TABLE class=\"table_list hover centerline\" width=$section_width cellspacing=0 cellpadding=1>\n";
	echo "<thead><TR>";
	echo "<TH>"._QXZ("Soundboard ID")."</TH>";
	echo "<TH>"._QXZ("Soundboard Name")."</TH>";
	echo "<TH>"._QXZ("Active")."</TH>";
	echo "<TH>"._QXZ("Layout")."</TH>";
	echo "<TH>"._QXZ("API User")."</TH>\n";
	echo "<TH>"._QXZ("ADMIN GROUP")."</TH>\n";
	echo "<th align=center><B>"._QXZ("MODIFY")."</B></td></tr>\n";
	echo "</TR></thead>\n";
	...
	while ($soundboard_to_print > $o) 
		...
		if (preg_match('/1$|3$|5$|7$|9$/i', $o))
			...
			{$bgcolor='';}
		echo "<tr $bgcolor"; if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$PHP_SELF?ADD=362111111111&soundboard_id=$row[0]&DB=$DB'\"";} echo "><td><a href=\"$PHP_SELF?ADD=362111111111&soundboard_id=$row[0]&DB=$DB\">$row[0]</a></td>";
		echo "<td>$row[1]</td>";
		echo "<td>"._QXZ("$row[2]")."</td>";
		echo "<td>$row[5]</td>";
		echo "<td>$row[3]</td>";
		echo "<td>".(preg_match('/\-\-ALL\-\-/', $row[4]) ? _QXZ("$row[4]") : $row[4])."</td>";
		echo "<td align=left><a class=\"btn alert-success\" href=\"$PHP_SELF?ADD=362111111111&soundboard_id=$row[0]&DB=$DB\">"._QXZ("MODIFY")."</a></td></tr>\n";
		$o++;
		}

	echo "</TABLE></center></div></div></div></div></div></div></div>\n";
	}
...
if ($ADD == "762000000000")
	...
	if ($LOGuser_level >= 9)
		...
		while ($logs_to_print > $o)
			...
			echo "<td> $row[1]</td>";
			echo "<td> <a href=\"admin.php?ADD=710000000000000&stage=$row[2]&DB=$DB\">$row[2]</a></td>";
			echo "<td> $row[3]</td>";
			echo "<td> $row[4]</td>";
			echo "<td> $row[5]</td>";
			echo "<td> $row[6]</td>";
			echo "<td> $row[7]</td>";
			echo "<td> <a href=\"$record_link\">"._QXZ("GOTO")."</a></td>";
			echo "</tr>\n";
			$logs_printed .= "'$row[0]',";
			$o++;
			}
