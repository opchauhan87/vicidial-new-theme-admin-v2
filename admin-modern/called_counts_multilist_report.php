...
$list_options="<select name='list_ids[]' id='list_ids' multiple size=4 class='form-control'>\n";
	if  (preg_match('/\-\-ALL\-\-/',$list_id_string))
...
$MAIN.='<div id="pcont4" class="container-fluid">
		...
		$MAIN.="<div class='col-md-12'><div class='content'>";
//~ $MAIN.="<TABLE CELLPADDING=4 CELLSPACING=0 class=block-flat><TR><TD>";
$MAIN.="<FORM ACTION=\"$PHP_SELF\" METHOD=GET name=vicidial_report id=vicidial_report>\n";
$MAIN.="<div class='header'><h3 class=\"pull-left\">"._QXZ("$report_name")."</h3><div class='pull-right'><a class=\"btn btn-info\" href=\"./admin.php?ADD=999999\">"._QXZ("BACK")."</a></div><div class='pull-right'>";
if (strlen($group[0]) > 1)
	{
	$MAIN.=" <a class='btn btn-info' href=\"./admin.php?ADD=34&campaign_id=$group[0]\">"._QXZ("MODIFY")."</a> \n";
	$MAIN.=" <a class='btn btn-info' href=\"./admin.php?ADD=999999\">"._QXZ("REPORTS")."</a> \n";
	$MAIN.=" <a class='btn btn-info' href=\"$PHP_SELF\">"._QXZ("RESET")."</a>";
	}
else
	{
	$MAIN.=" <a class=\"btn btn-info\" href=\"./admin.php?ADD=10\">"._QXZ("CAMPAIGNS")."</a> \n";
...
$MAIN.="<TABLE BORDER=0 CELLPADDING=3 CELLSPACING=0 BGCOLOR=\"#".$SSframe_background."\"><TR><TD class=\"col-md-4 text-left\" VALIGN=TOP>\n";
$MAIN.="<input type='checkbox' name='override_date' value='1' checked><label class='lbl'>"._QXZ("&nbsp;All dates")."</label></td></tr><TR><TD class=\"col-md-4\" VALIGN=TOP>";
$MAIN.="<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
$MAIN.="<INPUT TYPE=HIDDEN NAME=outbound_rate VALUE=\"$outbound_rate\">\n";
$MAIN.="<div class='col-lg-10 no-padding'><div class=\"form-group custom-input mb-10\"><INPUT class='form-control' TYPE=TEXT NAME=query_date SIZE=10 MAXLENGTH=10 VALUE=\"$query_date\"><label>"._QXZ("Dates")."</label></div></div>";
...
$MAIN.="<div class=\"col-md-12\" style=\"text-align:center;\"><label>"._QXZ("to")."</label></div>  <div class='col-md-12'><div class='col-lg-10 no-padding'><div class=\"form-group custom-input mb-10\"><INPUT class='form-control'  TYPE=TEXT NAME=end_date SIZE=10 MAXLENGTH=10 VALUE=\"$end_date\"></div></div>";
$MAIN.="<div class=\"col-md-2 text-center p-18\">";
$MAIN.="<script language=\"JavaScript\">\n";
$MAIN.="var o_cal = new tcal ({\n";
$MAIN.="	// form name\n";
...
$MAIN.="<div class=\"col-lg-12\"><div class='form-group custom-input mb-10'><SELECT multiple SIZE=5 NAME=group[] class=form-control id='group' $JS_events>\n";
if  (preg_match('/\-\-ALL\-\-/',$group_string))
...
$MAIN.="</SELECT><label> "._QXZ("Campaigns").":</label></div></div></td><td class='col-md-4' valign='TOP'>";
//$MAIN.="</TD><TD class='no-border' ALIGN=CENTER>";

//~ $MAIN.="</TD><TD valign='top' class='col-md-4'>";
$MAIN.="<div class=\"col-lg-12\"><div class='form-group custom-input mb-10'>\n";
$MAIN.=$list_options;
$MAIN.="<label>"._QXZ("Lists").":("._QXZ("optional, possibly slow").")</label></div></div></br>";
...
if (strlen($QUERY_STRING) > 5)
	...
	$MAIN.="<div class='col-md-12 mb-10'><center><b>"._QXZ("CALLED COUNTS WITHIN LIST(S)")." $list_id_title_str$date_range_title:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class='btn btn-bdr-blue' href=\"$PHP_SELF?DB=$DB$groupQS$list_idQS&query_date=$query_date&end_date=$end_date&override_date=$override_date&SUBMIT=$SUBMIT&file_download=1\">"._QXZ("DOWNLOAD")."</a></b></div>\n";
	$CSV_text.=_QXZ("CALLED COUNTS WITHIN LIST(S)")." $list_id_title_str$date_range_title:\n";
	$MAIN.="<pre class=\"m-0 spacer\"><TABLE  width=100% cellspacing=1 class=\"table_list hover centerline\" style='padding:3px !important ; '>\n";
	$MAIN.="<thead><tr><td align=left width=5px  style='padding:3px; !important'><b>"._QXZ("STATUS")."</b></td><td align=center width=5px  style='padding:3px; !important'><b>"._QXZ("STATUS NAME")."</b></td>";
	$CSV_text.="\""._QXZ("STATUS")."\",\""._QXZ("STATUS NAME")."\",";
	$first = $all_called_first;
	while ($first <= $all_called_last)
		{
		if (preg_match('/1$|3$|5$|7$|9$/i', $first)) {$AB='bgcolor=""';} 
		else{$AB='bgcolor=""';}
		if ($first >= 100) {$Fplus='+';}
		else {$Fplus='';}
		$MAIN.="<td align=center $AB width=3px  style='padding:3px; !important'><b>$first$Fplus</b></td>";
		$CSV_text.="\"$first$Fplus\",";
		$first++;
		}
	$MAIN.="<td align=center width=3px><font  style='padding:3px; !important'><b>"._QXZ("SUBTOTAL")."</b></td></thead></tr>\n";
	...
	while ($statuses_called_to_print > $sts) 
		...
		if (preg_match("/1$|3$|5$|7$|9$/i", $sts))
			{$bgcolor='bgcolor=""';   $AB='bgcolor=""';} 
		else
			{$bgcolor='bgcolor=""';   $AB='bgcolor=""';}
	#	$MAIN.="$status[$sts]|$status_called_first[$sts]|$status_called_last[$sts]|$leads_in_sts[$sts]|\n";
	#	$MAIN.="$status[$sts]|";
		$MAIN.="<tr $bgcolor><td width=3px  style='padding:3px; !important'>$Pstatus</td><td width=3px  style='padding:3px; !important'>$statuses_list[$Pstatus]</td>";
		...
		while ($first <= $all_called_last)
			...
			if (preg_match("/1$|3$|5$|7$|9$/i", $sts))
				...
				{
				if (preg_match("/0$|2$|4$|6$|8$/i", $first)) {$AB='bgcolor=""';} 
				else{$AB='bgcolor=""';}
				}
		...
		$MAIN.="<td>$leads_in_sts[$sts]</td></tr>\n\n";
		$CSV_text.="\"$leads_in_sts[$sts]\"\n";

		$sts++;
		}

	$MAIN.="<tr><td align=center colspan=2><b>"._QXZ("TOTAL")."</td>";
	$CSV_text.="\"\",\""._QXZ("TOTAL")."\",";
	$first = $all_called_first;
	while ($first <= $all_called_last)
		{
		...
		$MAIN.="<td align=center $AB><b>$all_called_count[$first]</td>";
		$CSV_text.="\"$all_called_count[$first]\",";
		$first++;
		}
	$MAIN.="<td align=center><b>$leads_in_list</td></tr>\n";
