...
while ($i < $VD_groups_ct)
	...
	if ($DB) {echo "$stmt\n";}
	if (!$rslt) {die('Could not execute 1: ' . mysqli_error());}
	$row=mysqli_fetch_row($rslt);
	...
	if ($group_found_count > 0)
		...
		$rslt=mysql_to_mysqli($stmt, $linkV);
		if ($DB) {echo "$stmt\n";}
		if (!$rslt) {die('Could not execute2: ' . mysqli_error());}
		$row=mysqli_fetch_row($rslt);
		$groupid = $row[0];
		$VTugID[$i] = $groupid;
		...
		if (!$rslt) {die('Could not execute3: ' . mysqli_error());}
	...
	### group doesn't exist in vtiger, insert it
		...
		$row=mysqli_fetch_row($rslt);
		$groupid = ($row[0] + 1);
		if (!$rslt) {die('Could not execute4: ' . mysqli_error());}
		$VTugID[$i] = $groupid;
		...
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_to_mysqli($stmt, $linkV);
		if (!$rslt) {die('Could not execute5: ' . mysqli_error());}

		echo $stmtA = "INSERT INTO vtiger_groups SET groupid='$groupid',groupname='$VTgroup_name',description='$VTgroup_description';";
		if ($DB) {echo "|$stmtA|\n";}
		$rslt=mysql_to_mysqli($stmtA, $linkV);
		if (!$rslt) {die('Could not execute6: ' . mysqli_error());}
...
while ($i < $VD_users_ct)
	...
	if ($found_count > 0)
		...
		$rslt=mysql_to_mysqli($stmt, $linkV);
		if ($DB) {echo "$stmt\n";}
		if (!$rslt) {die('Could not execute7: ' . mysqli_error());}
		$row=mysqli_fetch_row($rslt);
		$userid = $row[0];
		...
		$rslt=mysql_to_mysqli($stmt, $linkV);
		if ($DB) {echo "$stmt\n";}
		if (!$rslt) {die('Could not execute8: ' . mysqli_error());}
		$row=mysqli_fetch_row($rslt);
