...
if ($run_export > 0)
    ...
    {
    // Change to .xls or .xlsx extension
    $Excelfilename = "EXPORT_LEAD_REPORT_$FILE_TIME.xls";

    // Output as Excel file
    header('Content-Type: application/vnd.ms-excel');
    // Alternative for newer Excel: header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
<?php
	...
	//echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>"._QXZ("Export Leads Report")."</B></FONT><BR>\n";
		...
		echo "<div id=\"pcont4\" class=\"container-fluid\">
			  ...
			  <div class=\"header\">
			  <h3>"._QXZ("Export Leads Report")."</h3>
			  </div>";
	echo "<div class='size no-border spacer'>"._QXZ("This report pulls lead information for calls dialed in the selected date range. A lead is only exported once no matter how many calls were handled. The current lead status is used.")."</div>\n";
	echo "<FORM ACTION=\"$PHP_SELF\" METHOD=GET name=vicidial_report id=vicidial_report style='width:100%'>\n";
	echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">";
	echo "<INPUT TYPE=HIDDEN NAME=run_export VALUE=\"1\">";
	echo "<TABLE BORDER=0 CELLSPACING=8><TR><TD ALIGN=CENTER VALIGN=TOP  WIDTH=200>\n";
	
	echo "<div class='col-md-12'>";
	echo "<div class='col-md-10'><div class='form-group custom-input mb-10'><INPUT TYPE=TEXT NAME=query_date class=form-control SIZE=10 MAXLENGTH=10 VALUE=\"$query_date\"><label>"._QXZ("Date Range").":</label></div></div>";
	echo "<div class='col-md-2 p-18'>";
	?>
	...
	echo "</div></div></div><div class='col-md-12 text-center'><b>"._QXZ("to")."</b></div>\n";
	echo "<div class='col-md-12'><div class='col-md-10'><div class='form-group custom-input mb-10'><INPUT TYPE=TEXT NAME=end_date SIZE=10 class=form-control MAXLENGTH=10 VALUE=\"$end_date\"></div></div>";
	echo "<div class='col-md-2 p-18'>";
	?>
	<script language="JavaScript">
	var o_cal = new tcal ({
	...
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=date_field>";
	echo "<option selected value=\"call_date\">"._QXZ("Call date")."</option><option value=\"entry_date\">"._QXZ("Entry date")."</option>\n";
	echo "</SELECT><label>"._QXZ("Date Field").":</label></div>\n";
	
### bottom of first column
	//echo "<BR>\n";
	
	//echo "<B>"._QXZ("Header Row").":</B> $NWB#lead_export_report-header_row$NWE<BR>\n";
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=header_row>";
	echo "<option selected value=\"YES\">"._QXZ("YES")."</option><option value=\"NO\">"._QXZ("NO")."</option>\n";
	echo "</SELECT><label>"._QXZ("Header Row").":</label></div>\n";
	
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=rec_fields>";
	...
	echo "</SELECT><label>"._QXZ("Recording Fields").":</label></div>\n";
	...
	if ($custom_fields_enabled > 0)
		...
		echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=custom_fields>";
		echo "<option value='YES'>"._QXZ("YES")."</option><option value='NO' selected>"._QXZ("NO")."</option></select>\n";
		echo "</SELECT><label>"._QXZ("Custom Fields").":</label></div>\n";
		}
	...
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=call_notes>";
	echo "<option value='YES'>"._QXZ("YES")."</option><option value='NO' selected>"._QXZ("NO")."</option>\n";
	echo "</SELECT><label>"._QXZ("Per Call Notes").":</label></div>\n";
	
	//echo "<B>"._QXZ("Use DID filter").":</B> $NWB#lead_export_report-did_filter$NWE<BR>\n";
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'>";
	echo "<select class='form-control' size=1 name=custom_fields size=1 name=did_filter onChange=\"if (this.value=='YES') {document.getElementById('did_span').style.display='block';} else {document.getElementById('did_span').style.display='none';}\">";
	if ($did_filter) {echo "<option value='$did_filter' selected>$did_filter</option>";}
	echo "<option value='NO'>"._QXZ("NO")."</option><option value='YES'>"._QXZ("YES")."</option>\n";
	echo "</SELECT><label>"._QXZ("Use DID filter").":</label></div>\n";
	...
	echo "<div class='form-group custom-input mb-10'><SELECT  class='form-control'  NAME=campaign[] multiple>\n";
		$o=0;
	...
	echo "</SELECT><label>"._QXZ("Campaigns").":</label></div>\n";
	

	
	echo "<br><div class='form-group custom-input mb-10'><SELECT class='form-control' NAME=group[] multiple>\n";
		$o=0;
	...
	echo "</SELECT>";
	echo "<label>"._QXZ("Inbound Groups").":</label></div>\n";
	...
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=export_fields>";
	echo "<option value='STANDARD' selected>"._QXZ("STANDARD")."</option><option value='EXTENDED'>"._QXZ("EXTENDED")."</option>\n";
	echo "</SELECT><label>"._QXZ("Export Fields").":</label></div>\n";

	//echo "<BR><BR>\n";

	//echo "<B>"._QXZ("Exclude Call Log Data").":</B> $NWB#lead_export_report-exclude_call_log_data$NWE<BR>\n";
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=exclude_call_log_data>";
	echo "<option value='YES'>"._QXZ("YES")."</option><option value='NO' selected>"._QXZ("NO")."</option>\n";
	echo "</SELECT><label>"._QXZ("Exclude Call Log Data").":</label></div>\n";
		
	if ($archives_available=="Y") 
	{
	
	//echo "<BR><BR><B>"._QXZ("Search archived data").":</B> $NWB#lead_export_report-search_archived_data$NWE<BR>\n";
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=search_archived_data>";
		...
		echo "</select>\n";
		echo "<label>"._QXZ("Search archived data").":</label></div>\n";
	}
	
	echo "</TD><TD class='col-md-4' VALIGN=TOP >\n";
	
	
	echo "<div class='form-group custom-input mb-10'><SELECT class='form-control' NAME=list_id[] multiple>\n";
		$o=0;
	...
	echo "</SELECT><label>"._QXZ("Lists").":</label></div><br>\n";
	
	echo "<div class='form-group custom-input mb-10'><SELECT class='form-control'  NAME=status[] multiple>\n";
		$o=0;
	...
	echo "</SELECT><label>"._QXZ("Statuses").":</label></div>\n";
	echo "<BR>\n";

	
	
	echo "<div class='form-group custom-input mb-10'><SELECT class='form-control' NAME=user_group[] multiple>\n";
		$o=0;
	...
	echo "</SELECT><label>"._QXZ("User Groups").":</label></div>";
	
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=call_notes><option value='YES'>"._QXZ("YES")."</option><option value='NO' selected>"._QXZ("NO")."</option></select><label>"._QXZ("Per Call Notes").":</label></div></div>\n";
	echo "<div class='col-md-12'><div class='form-group custom-input mb-10'><select class='form-control' size=1 name=export_fields><option value='STANDARD' selected>"._QXZ("STANDARD")."</option><option value='EXTENDED'>"._QXZ("EXTENDED")."</option></select><label>"._QXZ("Export Fields").":</label></div></div>\n";
	
	echo "<BR><BR>\n";

	
	
	echo "<div ID='did_span' style='display:".($did_filter=="YES" ? "block" : "none").";'>\n";
	echo "<font class=\"select_bold\"><B>"._QXZ("DIDs").":</B></font><BR><CENTER>\n";
	echo "<div class='form-group custom-input mb-10'><SELECT SIZE=20 NAME=dids[] multiple>\n";
		$o=0;
	...
	echo "<div ID='vlc_span' style='display:".($vlc_filter=="YES" ? "block" : "none").";'>\n";
	echo "<font class=\"select_bold\"><B>"._QXZ("Vendor lead codes").":</B></font><BR><CENTER>\n";
	echo "<div class='form-group custom-input mb-10'><SELECT SIZE=20 NAME=vlcs[] multiple>\n";
		$o=0;
...
echo "<INPUT TYPE=SUBMIT NAME=SUBMIT VALUE='"._QXZ("SUBMIT")."' class='btn btn-success'>\n";
		echo "</TD></TR>\n";
