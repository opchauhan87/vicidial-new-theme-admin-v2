...
if (isset($_GET["fullname"]))					{$user=$_GET["fullname"];}
	elseif (isset($_POST["fullname"]))			{$user=$_POST["fullname"];}
	...
	if ($found_count < 1)
		...
		if ( ($deleted_count > 0) and (preg_match('/DISABLED/',$vtiger_search_dead)) )
			...
			if (preg_match('/Y/',$vtiger_create_call_record))
				...
				echo $stmt="SELECT id from vtiger_users where user_name='$user';";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_to_mysqli($stmt, $linkV);
				$row=mysqli_fetch_row($rslt);
				$user_id = $row[0];
				...
				echo $stmt = "INSERT INTO vtiger_salesmanactivityrel SET smid='$user_id',activityid='$activityid';";
				if ($DB) {echo "|$stmt|\n";}
				$rslt=mysql_to_mysqli($stmt, $linkV);
				if ($DB) {echo "|$leadid|\n";}
				if (!$rslt) {die('Could not execute42: ' . mysqli_error());}
				...
				$account_URL = "$vtiger_url/index.php?module=Calendar&view=Edit&return_module=Accounts&return_action=DetailView&record=$activityid&activity_mode=Events&return_id=$vendor_id&app=SALES";
				}
			else
				{
				# http://mysite.com/vtigercrm/index.php?module=Accounts&action=DetailView&record=2&parenttab=Sales
				$account_URL = "$vtiger_url/index.php?module=Accounts&action=DetailView&record=$vendor_id&app=SALES";
				}
	...
	if (strlen($accountid) > 0)
		...
		if ( ($deleted_count > 0) and (preg_match('/DISABLED/',$vtiger_search_dead)) )
			...
			if (preg_match('/Y/',$vtiger_create_call_record))
				...
				# http://mysite.com/vtigercrm/index.php?module=Calendar&action=EditView&return_module=Accounts&return_action=DetailView&record=16&activity_mode=Events&return_id=9&parenttab=Sales
				$account_URL = "$vtiger_url/index.php?module=Calendar&view=Edit&return_module=Accounts&return_action=DetailView&record=$activityid&activity_mode=Events&return_id=$accountid&app=SALES";
				}
			else
				{
				# http://mysite.com/vtigercrm/index.php?module=Accounts&action=DetailView&record=2&parenttab=Sales
				$account_URL = "$vtiger_url/index.php?module=Accounts&action=DetailView&record=$accountid&app=SALES";
				}
			echo "<META HTTP-EQUIV=Refresh CONTENT=\"0; URL=$account_URL\">\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0\">\n";
...
if ($vendor_search > 0)
	...
	if ($found_count < 1)
		...
		$account_URL = "$vtiger_url/index.php?module=Vendors&action=DetailView&record=$vendorid&app=SALES";
		echo "<META HTTP-EQUIV=Refresh CONTENT=\"0; URL=$account_URL\">\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0\">\n";
		echo "<CENTER><FONT FACE=\"Courier\" COLOR=BLACK SIZE=3>\n";
	...
	if ($found_count < 1)
		...
		if (preg_match('/Y/',$vtiger_create_lead_record))
			...
			if (preg_match('/Y/',$vtiger_create_call_record))
				...
				# http://mysite.com/vtigercrm/index.php?module=Calendar&action=EditView&return_module=Accounts&return_action=DetailView&record=16&activity_mode=Events&return_id=9&parenttab=Sales
				$account_URL = "$vtiger_url/index.php?module=Calendar&view=Edit&return_module=Leads&return_action=DetailView&record=$activityid&activity_mode=Events&return_id=$leadid&app=SALES";
				...
				$account_URL = "$vtiger_url/index.php?module=Leads&view=Edit&record=$leadid&app=SALES";
				}
		...
		if ( ($deleted_count > 0) and (preg_match('/DISABLED/',$vtiger_search_dead)) )
			...
			if (preg_match('/Y/',$vtiger_create_call_record))
				...
				{
				# http://mysite.com/vtigercrm/index.php?module=Accounts&action=DetailView&record=2&parenttab=Sales
				##$account_URL = "$vtiger_url/index.php?module=Leads&action=DetailView&record=$leadid&app=sales";
				$account_URL = "$vtiger_url/index.php?module=Leads&view=Detail&record=$leadid&app=SALES";
				}
			echo "<META HTTP-EQUIV=Refresh CONTENT=\"0; URL=$account_URL\">\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0\">\n";
