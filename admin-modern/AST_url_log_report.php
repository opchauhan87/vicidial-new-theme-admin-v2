...
$MAIN.='<div id="pcont4" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3>'._QXZ("$report_name").'</h3></div><div class="col-md-12"><div class="content">';
//$MAIN.="<TABLE CELLPADDING=4 CELLSPACING=0><TR><TD>";
$MAIN.="<FORM ACTION=\"$PHP_SELF\" METHOD=GET name=vicidial_report id=vicidial_report>\n";
$MAIN.="<TABLE BORDER=0 cellspacing=5 cellpadding=5 class='col-lg-12'><TR><TD VALIGN=TOP class='col-lg-4 no-border size'>\n";
$MAIN.="<div class='col-lg-12 size'><INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
$MAIN.= "</div><div class='col-lg-12'><div class='col-lg-5 no-padding'>";
$MAIN.="<div class=\"form-group custom-input mb-10\"><INPUT TYPE=TEXT NAME=query_date SIZE=10 class='form-control' MAXLENGTH=10 VALUE=\"$query_date\">";
$MAIN.="<label>"._QXZ("Date")."</label></div></div><div class='col-lg-1 no-padding'><script language=\"JavaScript\">\n";
...
$MAIN.="</div></br></br>";
$MAIN.="<br><div class='col-lg-12 no-padding'><div class='col-lg-5 no-padding'><INPUT TYPE=TEXT NAME=query_date_D SIZE=9 class='form-control' MAXLENGTH=8 VALUE=\"$query_date_D\">";
$MAIN.="</div><div class='col-md-1 size'>"._QXZ("to")."</div><div class='col-lg-5 no-padding'><INPUT TYPE=TEXT NAME=query_date_T SIZE=9 class='form-control' MAXLENGTH=8 VALUE=\"$query_date_T\">";

$MAIN.="</div></div></TD><TD VALIGN=TOP class='no-border size col-lg-4'>";
$MAIN.="<div class='col-lg-8 no-padding'><div class=\"form-group custom-input mb-10\"><SELECT SIZE=5 NAME=url_type[] class='form-control' multiple>\n";
if  (preg_match('/--ALL--/',$url_type_string))
...
$MAIN.="</SELECT><label>"._QXZ("URL type")."</label></div></div></TD><TD VALIGN=top class='no-border size col-lg-4'>\n";
//~ $MAIN.=_QXZ("Display as").":<BR>";
$MAIN.="<div class='col-lg-4 no-padding'><div class=\"form-group custom-input mb-10\"><select name='report_display_type' class='form-control'>";
...
$MAIN.="</TD></TR><TR><TD class=text-center colspan=3 class='no-border'><INPUT TYPE=submit NAME=SUBMIT VALUE='"._QXZ("SUBMIT")."' align=center class='btn btn-bdr-sucess'><BR/><BR/>\n";
$MAIN.="</TD></TR></TABLE>\n";
if ($SUBMIT && $url_type_ct>0) {
	...
	if (mysqli_num_rows($rslt)>0) {
		...
		$HTML_text.="<label>"._QXZ("URL TYPE BREAKDOWN FOR")." $query_date, $query_date_D "._QXZ("TO")." $query_date_T $server_rpt_string</label>";
		$HTML_text.="<table class='table_list hover centerline' border='0' cellpadding='0' cellspacing='2' width='350'>";
		$HTML_text.="<thead><TR><TH>"._QXZ("URL TYPE")."</th><TH>"._QXZ("COUNT")."</th></tr></thead>";
		...
		while ($row=mysqli_fetch_array($rslt)) {
			...
			$ASCII_text.="|\n";
			$HTML_text.="<TR><TD>$row[url_type]</td><TD class='small_standard'>$row[ct]</td></tr>";
			$total_count+=$row["ct"];
		}
		$ASCII_text.="+--------------+---------+\n";
		$ASCII_text.="| "._QXZ("TOTAL",12,"r")." | ".sprintf("%-8s", $total_count)."|\n";
		$ASCII_text.="+--------------+---------+\n\n";
		$HTML_text.="<TR><TH>"._QXZ("TOTAL")."</th><TH class=''>$total_count</th></tr></table>";
		...
		$HTML_rpt.="<label>"._QXZ("URL LOG RECORDS FOR")." $query_date, $query_date_D "._QXZ("TO")." $query_date_T $server_rpt_string, "._QXZ("RECORDS")." #$lower_limit-$upper_limit<a href=\"$PHP_SELF?SUBMIT=$SUBMIT&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$url_typeQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">["._QXZ("DOWNLOAD")."]</a></label>";
		$HTML_text.="<BR><BR><table class='table_list hover centerline' border='0' cellpadding='0' cellspacing='2' width='1000'>";
		$HTML_rpt.="<thead><TR><TH>"._QXZ("UNIQUE ID")."</TH><TH>"._QXZ("URL DATE")."</TH><TH>"._QXZ("URL TYPE")."</TH><TH>"._QXZ("RESP SEC")."</TH><TH>"._QXZ("URL")."</TH><TH>"._QXZ("URL RESPONSE")."</TH></TR></thead>";
		...
		for ($i=1; $i<=mysqli_num_rows($rpt_rslt); $i++) {
			...
			if ($i>=$lower_limit && $i<=$upper_limit) {
				...
				$HTML_rpt.="<TR valign='top'><td><div style='width: 90px' class='wordwrap'>$row[uniqueid]</div></td><td class='' width='120'><div style='width: 120px'>$row[url_date]</div></td><td class='' width='70'><div style='width: 70px'>$row[url_type]</div></td><td class='' width='70'><div style='width: 70px'>$row[response_sec]</div></td><td class='' width='350'><div style='width: 350px' class='wordwrap'>$row[url]</div></td><td class='' width='300'><div style='width: 300px' class='wordwrap'>$row[url_response]</div></td></TR>";
