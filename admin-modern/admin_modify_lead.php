...
if ( ($auth_message == 'GOOD') or ($auth_message == '2FA') )
	...
	if ($auth_message == '2FA')
		...
		echo _QXZ("Your session is expired").". <a class='btn btn-primary btn-sm' href=\"admin.php\">"._QXZ("Click here to log in")."</a>.\n";
		exit;
		}
	}
...
<?php
		...
		$HTML_text.= "<div class='cl-mcont'>";
		$HTML_text.= "<div class='row'>";
		$HTML_text.= "<div class='col-md-12 no-padding'>";
		$HTML_text.= "<div class='block-flat'>";
		//$HTML_text.="<TABLE width=1000 cellspacing=1 cellpadding=5>\n";
		//$HTML_text.="<tr>\n";
		$HTML_text.= "<div class='header'>";
		$HTML_text.="<h3>"._QXZ("GDPR DATA PURGE PREVIEW")."</h3> <I>("._QXZ("greyed fields will be emptied").")</I>:";
		$HTML_text.="<a class='btn btn-primary' href=\"admin_modify_lead.php?lead_id=$lead_id&gdpr_action=download\">"._QXZ("DOWNLOAD")."</a>";
		$HTML_text.="<a class='btn btn-primary' href=\"admin_modify_lead.php?lead_id=$lead_id&gdpr_action=confirm_purge\">"._QXZ("CONFIRM PURGE")."</a>";
		//$HTML_text.="</tr>";
		...
		for ($t=0; $t<$table_count; $t++) 
			...
			if (mysqli_num_rows($mysql_rslt)>0)
				...
				$HTML_text.= "<div class='col-md-12 no-padding'>";
				$HTML_text.= "<div class='block-flat'>";
				$HTML_text.= "<div class='col-md-12'>";
				$HTML_text.= "<div class='content'>";
				//$HTML_text.="<TABLE width=1000 cellspacing=1 cellpadding=5>\n";
				$HTML_text.="<B>"._QXZ("TABLE NAME").": $table_name</B><BR>\n";
				$HTML_text.= "<table class='table hover' width='600' style='width:100%;overflow-x:scroll;overflow-y:auto;display:block;'>";
				$j=0;
				while($row=mysqli_fetch_array($mysql_rslt,MYSQLI_ASSOC)) 
					...
					$CSV_text.="\n";
					$HTML_text.="</table>";
					$HTML_text.="</div></div></div></div></div></div></div>";
				}
...
echo "<span id=agent_status_stats>\n";
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	while ($logs_to_print > $u) 
		...
		$call_log .= "<td ><font size=2> $row[7]</td>\n";
		$call_log .= "<td ><font size=2> $row[8]</td>\n";
		$call_log .= "<td ><font size=2> <A HREF=\"user_stats.php?user=$row[11]\" target=\"_blank\">$row[11]</A> </td>\n";
		$call_log .= "<td><font size=2> $row[3] </td>\n";
		$call_log .= "<td><font size=2> $row[2] </td>\n";
		$call_log .= "<td ><font size=2> $row[1] </td>\n";
		$call_log .= "<td ><font size=2> $row[15] </td>\n";
		$call_log .= "<td ><font size=2>&nbsp; $row[10] </td>\n";
		//end
		...
		if ($CIDdisplay=="Yes")
			...
			//chirag
			//16-03-21
			$stmtA="SELECT caller_code,server_ip FROM vicidial_log_extended WHERE lead_id='" . mysqli_real_escape_string($link, $lead_id) . "' and uniqueid='$uniqueid';";
...
$outbound_cid_num='';   $outbound_cid_type='';
	...
	while ($Alogs_to_print > $y) 
		...
		$agent_log .= "<td><font size=2> $row[5]</td>\n";
		$agent_log .= "<td ><font size=2> <A HREF=\"user_stats.php?user=$row[1]\" target=\"_blank\">$row[1]</A> </td>\n";
		$agent_log .= "<td ><font size=2> $row[7]</td>\n";
		$agent_log .= "<td><font size=2> $row[9] </td>\n";
		$agent_log .= "<td ><font size=2> $row[11] </td>\n";
		$agent_log .= "<td ><font size=2> $row[13] </td>\n";
		$agent_log .= "<td><font size=2> &nbsp; $row[14] </td>\n";
		$agent_log .= "<td><font size=2> &nbsp; $row[15] </td>\n";
		$agent_log .= "<td ><font size=2> &nbsp; $row[17] </td></tr>\n";
		...
		if ($in_notes_to_print > 0)
			...
			if ($in_notes_to_print > 0)
				...
				if (strlen($rowA[0]) > 0)
					...
					$closer_log .= "<TR>";
					$closer_log .= "<td></td>";
					$closer_log .= "<TD COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"> "._QXZ("NOTES").": &nbsp; $rowA[0] </font></TD>";
					$closer_log .= "</TR>";
					}
				}
			}
	...
	#	$Clog_campaign = '';
		while ($Clogs_to_print > $y) 
			...
			$closer_log .= "<td><font size=2> $row[7]</td>\n";
			$closer_log .= "<td><font size=2> $row[8]</td>\n";
			$closer_log .= "<td><font size=2> <A HREF=\"user_stats.php?user=$row[11]\" target=\"_blank\">$row[11]</A> </td>\n";
			$closer_log .= "<td><font size=2> $row[3] </td>\n";
			$closer_log .= "<td><font size=2> $row[2] </td>\n";
			$closer_log .= "<td><font size=2> $row[1] </td>\n";
			$closer_log .= "<td><font size=2> &nbsp; $row[14] </td>\n";
			$closer_log .= "<td><font size=2> &nbsp; $row[17] </td></tr>\n";
			//end
		...
		#	$Alog_campaign = '';
			while ($Alogs_to_print > $y) 
				...
				if ( ($LOGmodify_leads == '3') or ($LOGmodify_leads == '4') )
					{
					$agent_log .= "<td align=left><font size=2> &nbsp; <font color=blue><u><span id=\"vicidial_agent_log_archive_status_$y\" onClick=\"ModifyLogDisplayShow(event, $y,'vicidial_agent_log_archive','$uniqueid','$row[4]','$row[3]','$row[14]','$row[1]')\">$row[14]</span></u></font></td>\n";
		...
		#	$Clog_campaign = '';
			while ($Clogs_to_print > $y) 
				...
				if ( ($LOGmodify_leads == '3') or ($LOGmodify_leads == '4') )
					{
					$closer_log .= "<td align=left><font size=2><font color=blue><u><span id=\"vicidial_closer_log_archive_status_$y\" onClick=\"ModifyLogDisplayShow(event, $y,'vicidial_closer_log_archive','$uniqueid','$row[1]','$row[4]','$row[8]','$row[11]')\">$row[8]</span></u></font></td>\n";
...
//end
	...
	if ($lead_id == 'NEW')
		...
		$Lc=0;
		$select_list = '<div class=col-md-3><select class=form-control size=1 name=list_id>';
...
echo '<div class="header">';
	...
	if ($lead_id == 'NEW')
		...
		echo "<br><h3>"._QXZ("Add A New Lead")."</h3>\n";
		}
	else
...
echo '</div>';
	...
	if ( ($lead_count > 0) or ($lead_id == 'NEW') )
		...
		echo "<tr><td style='text-align:left;' colspan=2><div class='col-md-3'>$label_lead_id: $lead_id &nbsp; &nbsp;</div><div class=col-md-1> $label_list_id:</div>  $list_id &nbsp; &nbsp; <font  size=2>$label_gmt_offset_now: $gmt_offset_now &nbsp; &nbsp; $label_called_since_last_reset: $called_since_last_reset</td></tr>\n";
		echo "<tr><td style='text-align:left;' colspan=2>$label_user: <A HREF=\"user_stats.php?user=$tsr\">$tsr</A> &nbsp; &nbsp; $label_called_count: $called_count <font size=2>("._QXZ("today").": $call_count_today)</font> &nbsp; &nbsp; $label_last_local_call_time: $last_local_call_time</td></tr>\n";
		...
		if ($LOGadmin_hide_lead_data != '0')
			...
			{
			echo "<tr><td align=right>$label_title: </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=title id=title size=4 maxlength=$MAXtitle value=\"$title\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_first_name : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=first_name id=first_name size=15 maxlength=$MAXfirst_name value=\"".htmlparse($first_name)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_middle_initial:  </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=middle_initial id=middle_initial size=4 maxlength=$MAXmiddle_initial value=\"".htmlparse($middle_initial)."\"></div></td></tr> \n";
			
			echo "<tr><td align=right>$label_last_name : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=last_name id=last_name size=15 maxlength=$last_name value=\"".htmlparse($last_name)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_address1 : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=address1 id=address1 size=40 maxlength=$MAXaddress1 value=\"".htmlparse($address1)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_address2 : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=address2 id=address2 size=40 maxlength=$MAXaddress2 value=\"".htmlparse($address2)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_address3 : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=address3 id=address3 size=40 maxlength=$MAXaddress3 value=\"".htmlparse($address3)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_city : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=city id=city size=40 maxlength=$MAXcity value=\"".htmlparse($city)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_state: </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=state id=state size=2 maxlength=$MAXstate value=\"".htmlparse($state)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_postal_code : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=postal_code id=postal_code size=10 maxlength=$MAXpostal_code value=\"".htmlparse($postal_code)."\"></div></td></tr>\n";

			echo "<tr><td align=right>$label_province : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=province id=province size=40 maxlength=$MAXprovince value=\"".htmlparse($province)."\"></div></td></tr>\n";
			
			echo "<tr><td align=right>$label_country_code : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=country_code id=country_code size=3 maxlength=$MAXcountry_code value=\"".htmlparse($country_code)."\"></div></td></tr> \n";
			
			//echo " $label_date_of_birth: <input class='form-control' type=text name=date_of_birth id=date_of_birth size=12 maxlength=10 value=\"".htmlparse($date_of_birth)."\"></td></tr>\n";
			echo "<tr><td align=right>$label_date_of_birth : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=date_of_birth id=date_of_birth size=12 maxlength=10 value=\"".htmlparse($date_of_birth)."\"></div></td></tr>\n";

			if ( ($LOGmodify_leads == '1') or ($LOGmodify_leads == '3') or ($lead_id == 'NEW') )
				{
				echo "<tr><td align=right>$label_phone_number : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=phone_number id=phone_number size=18 maxlength=$MAXphone_number value=\"".htmlparse($phone_number)."\"></div></td></tr>\n";
				echo "<tr><td align=right>$label_phone_code : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=phone_code id=phone_code size=10 maxlength=$MAXphone_code value=\"".htmlparse($phone_code)."\"></div></td></tr>\n";
				echo "<tr><td align=right>$label_alt_phone : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=alt_phone id=alt_phone size=12 maxlength=$MAXalt_phone value=\"".htmlparse($alt_phone)."\"></div></td></tr>\n";
				}
			else
				{
				echo "<tr><td align=right>$label_phone_number : </td><td align=left><div class='col-lg-5'><input class='form-control' type=hidden name=phone_number value=\"".htmlparse($phone_number)."\">".htmlparse($phone_number)."</div></td></tr>\n";
				echo "<tr><td align=right>$label_phone_code : </td><td align=left><div class='col-lg-5'><input class='form-control' type=hidden name=phone_code value=\"".htmlparse($phone_code)."\">".htmlparse($phone_code)."</div></td></tr>\n";
				echo "<tr><td align=right>$label_alt_phone : </td><td align=left><div class='col-lg-5'><input class='form-control' type=hidden name=alt_phone value=\"".htmlparse($alt_phone)."\">".htmlparse($alt_phone)."</div></td></tr>\n";
				}

			echo "<tr><td align=right>$label_email : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=email size=40 maxlength=$MAXemail value=\"".htmlparse($email)."\"></div></td></tr>\n";
			echo "<tr><td align=right>$label_security_phrase : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=security id=security_phrase size=30 maxlength=$MAXsecurity_phrase value=\"".htmlparse($security)."\"></div></td></tr>\n";
			echo "<tr><td align=right>$label_vendor_lead_code : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=vendor_id id=vendor_lead_code size=30 maxlength=$MAXvendor_lead_code value=\"".htmlparse($vendor_id)."\"></div></td></tr>\n";
			if ($SSsource_id_display > 0)
				{echo "<tr><td align=right>$label_source_id : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=source_id id=source_id size=30 maxlength=$MAXsource_id value=\"".htmlparse($source_id)."\"></div></td></tr>\n";}
			echo "<tr><td align=right>$label_rank : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=rank id=rank size=7 maxlength=5 value=\"".htmlparse($rank)."\"></div></td></tr>\n";
			echo "<tr><td align=right>$label_owner : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name=owner id=owner size=22 maxlength=$MAXowner value=\"".htmlparse($owner)."\"></div></td></tr>\n";
			echo "<tr><td align=right>$label_comments : </td><td align=left><div class='col-lg-5'><TEXTAREA class='form-control' name=comments id=comments ROWS=3 COLS=65>".htmlparse($comments)."</TEXTAREA></div></td></tr>\n";
			}
		}
		else
		{
	...
	if ($lead_id != 'NEW') 
		...
		while ($row_count > $o)
			...
			//ishwari
			echo "<tr><td align=right class='col-sm-3'><font size=2>$Atimestamp[$o]: </td><td align=left><div class='col-md-6' style='padding-left:10px;'><font size=2><hr> &nbsp; $Acomment[$o]<br> &nbsp; </font><font size=1><i>by user: $Auser[$o] - $Afull_name</i></div></td></tr>\n";
			//end
			$o++;
			}
		...
		//ishwari
		echo "<tr bgcolor=''><td align=right class='col-sm-3'>"._QXZ("Disposition").": </td><td align=left><div class='col-md-6' style='padding-left:10px;'><select class='form-control' size=1 name=status>\n";
		...
		echo "$statuses_list";
		echo "</select></div> <i>("._QXZ("with")." $list_campaign "._QXZ("statuses").")</i></td></tr>\n";

		//ishwari
		echo "<tr bgcolor=''><td  align=left class='col-sm-3'>"._QXZ("Modify vicidial log")." </td><td align=left><div class='col-md-2' style='padding-left:10px;'><input type=checkbox name=modify_logs value=\"1\" CHECKED></div></td></tr>\n";
		
		echo "<tr bgcolor=''><td align=left class='col-sm-3'>"._QXZ("Modify agent log")." </td><td align=left><div class='col-md-2' style='padding-left:10px;'><input type=checkbox name=modify_agent_logs value=\"1\" CHECKED></div></td></tr>\n";
		
		echo "<tr bgcolor=''><td align=left class='col-sm-3'>"._QXZ("Modify closer log")." </td><td align=left><div class='col-md-2' style='padding-left:10px;'><input type=checkbox name=modify_closer_logs value=\"1\"></div></td></tr>\n";
		
		echo "<tr bgcolor=''><td align=left class='col-sm-3'>"._QXZ("Add closer log record")." </td><td align=left><div class='col-md-2' style='padding-left:10px;'><input type=checkbox name=add_closer_record value=\"1\"></div></td></tr>\n";
		//end
		}
	else
		{
	...
	if ( ($LOGadmin_hide_lead_data == '0') or ($lead_id == 'NEW') )
		{
		echo "<tr><td colspan=2 class=text-center><input type=submit  class='btn btn-bdr-sucess' name=submit value=\""._QXZ("SUBMIT")."\"></td></tr>\n";
		}
	echo "</table></form>\n";
	echo "<BR><BR><BR>\n";
	//ishwari
	...
	if ($lead_id != 'NEW') 
		{
		echo "<TABLE BGCOLOR=#".$SSstd_row4_background." WIDTH=750><TR><TD class='text-left'>\n";
		echo "<b>"._QXZ("Callback Details").":</b><BR>\n";
		if ( ($dispo == 'CALLBK') or ($dispo == 'CBHOLD') or ($scheduled_callback == 'Y') )
			...
			if ($CB_to_print>0)
				...
				echo "<TABLE BGCOLOR=#".$SSstd_row4_background." WIDTH=750>";
				echo "<tr>";
				...
				while ($cb_row=mysqli_fetch_row($cb_rslt)) {
					...
					echo "<tr>";
					echo "<td><div class='form-group'><div class='col-md-8'><input name='appointment_date_".$callback_id."' id='appointment_date_".$callback_id."' class='form-control' maxlength=10 value='$appointment_date'></div><div class='col-md-3' style='left: -10px;top:7px;'>";
					echo "					<script language=\"JavaScript\">
					var o_cal = new tcal ({
						// form name
						'formname': 'vsn',
					...
					echo "</div></div></br>";
					echo "<BR>";
					echo "<div class='form-group'><div class='col-sm-6'><SELECT class='form-control'  name='appointment_hour_".$callback_id."' id='appointment_hour_".$callback_id."'>\n";
					for ($i=0; $i<=23; $i++) {
						$hr=substr("0$i", -2);
					...
					echo "</SELECT></div>";
					//echo ":";
					echo "<div class='col-sm-6'><SELECT class='form-control' name='appointment_min_".$callback_id."' id='appointment_min_".$callback_id."'>";
					for ($i=0; $i<=55; $i+=5) {
						$min=substr("0$i", -2);
					...
					echo "</SELECT></div></div>";
					echo "</td>";
					echo "<td><FONT FACE='ARIAL,HELVETICA'><select class='form-control' size=1 name='CBstatus_".$callback_id."' id='CBstatus_".$callback_id."'>\n";
					echo "$statuses_list";
					echo "</td>";
					echo "<td><font size='2'>";
					...
					echo _QXZ("New CB Owner UserID").":<BR><input type=text class='form-control' name='CBuser_".$callback_id."' id='CBuser_".$callback_id."' size=6 maxlength=20 value=\"$cb_row[8]\"> \n";
					echo "<input type=hidden id='recipient_".$callback_id."' name='recipient_".$callback_id."' value=\"YES\">";
					echo "</font></td>";
					echo "<td><TEXTAREA class='form-control' name='comments_".$callback_id."' id='comments_".$callback_id."' ROWS=6 COLS=50>$CBcomments</TEXTAREA></td>";
					echo "<td><input type='button' class='btn btn-bdr-sucess' value='UPDATE' onClick='UpdateCallback($callback_id)'></td>";
					echo "</tr>";
			...
			while ($CB_to_print > $cb)
				...
				$callbacks_log .= "</tr><tr>";
				$callbacks_log .= "<td><font size=1> &nbsp; </td>";
				$callbacks_log .= "<td colspan=8 align=left><font size=2>"._QXZ("Comments").": </font><font size=2>$rowx[8]</font></td>";
				$callbacks_log .= "</tr>\n";
				}
			...
			if ($cb > 0)
				...
				echo "<B>"._QXZ("CALLBACKS LOG").":</B>\n";
				echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
				echo "<tr><th><font size=1># </th><th><font size=2>"._QXZ("ENTRY TIME")." </th><th><font size=2>"._QXZ("CALLBACK TIME")." </th><th align=left><font size=2>"._QXZ("USER")."</th><th align=left><font size=2> "._QXZ("RECIPIENT")."</th><th align=left><font size=2> "._QXZ("LEAD STATUS")."</th><th align=left><font size=2> "._QXZ("STATUS")."</th><th align=left><font size=2> "._QXZ("LIST")."</th><th align=left><font size=2> "._QXZ("CAMPAIGN")."</th></tr>\n";
		...
		//ishwari
		//27-jun-19
		echo "<center><B>"._QXZ("CALLS TO THIS LEAD").":</B></center>\n";
		//end
		//chirag
		//16-03-21
		if ($CIDdisplay=="Yes")
		...
		//ishwari
		//27-jun-19
		echo "<center><B>"._QXZ("CLOSER RECORDS FOR THIS LEAD").":</B></center>\n";
		//chirag
		//16-03-21
			if ($CIDdisplay=="Yes")
			{
		...
		//ishwari
		//27-jun-19
		echo "<center><B>"._QXZ("AGENT LOG RECORDS FOR THIS LEAD").":</B></center>\n";
		echo "<TABLE width=750 cellspacing=0 cellpadding=1>\n";
		echo "<tr><th><font size=1># </th><th><font size=2>"._QXZ("DATE/TIME")." </th><th align=left><font size=2>"._QXZ("CAMPAIGN")."</th><th align=left><font size=2> "._QXZ("TSR")."</th><th align=left><font size=2> "._QXZ("PAUSE")."</th><th align=right><font size=2> "._QXZ("WAIT")."</th><th align=right><font size=2> "._QXZ("TALK")."</th><th align=right><font size=2> "._QXZ("DISPO")."</th><th align=right><font size=2> "._QXZ("STATUS")."</th><th align=right><font size=2> "._QXZ("GROUP")."</th><th align=right><font size=2> "._QXZ("SUB")."</th></tr>\n";
		//end
			echo "$agent_log\n";
		...
		//ishwari
		//27-jun-19
		echo "<center><B>"._QXZ("PARK LOGS FOR THIS LEAD").":</B></center>\n";
		echo "<TABLE width=750 cellspacing=1 cellpadding=1>\n";
		echo "<tr><th><font size=1># </th><th align=left><font size=2> "._QXZ("PARK TIME")."</th><th><font size=2>"._QXZ("CHANNEL GROUP")." </th><th align=left><font size=2>"._QXZ("TSR")." </th><th align=left><font size=2> &nbsp; "._QXZ("STATUS")."</th><th align=left><font size=2> "._QXZ("GRAB TIME")."</th><th align=left><font size=2> "._QXZ("HANGUP TIME")."</th><th align=left><font size=2> "._QXZ("PARK SEC")."</th><th align=left><font size=2> "._QXZ("TALK SEC")."</th><th align=left><font size=2> "._QXZ("EXTENSION")."</th></tr>\n";
		//end
		...
		while ($logs_to_print > $u) 
			...
			echo "<td><font size=1> $row[parked_time] </td>";
			echo "<td><font size=1> $row[channel_group] </td>\n";
			echo "<td><font size=2> $row[user] </td>\n";
			echo "<td><font size=2> $row[status] </td>\n";
			echo "<td><font size=1> $row[grab_time] </td>\n";
			echo "<td><font size=1> $row[hangup_time] </td>\n";
			echo "<td><font size=2> $row[parked_sec] </td>\n";
			echo "<td><font size=2> $row[talked_sec] </td>\n";
			echo "<td><font size=1> $row[extension] </td>\n";
			echo "</tr>\n";
			//end
			}
		...
		//ishwari
		//27-jun-19
		echo "<center><B>"._QXZ("IVR LOGS FOR THIS LEAD").":</B></center>\n";
		echo "<TABLE width=750 cellspacing=1 cellpadding=1>\n";
		echo "<tr><th><font size=1># </th><th align=left><font size=2> "._QXZ("CAMPAIGN")."</th><th><font size=2>"._QXZ("DATE/TIME")." </th><th align=left><font size=2>"._QXZ("CALL MENU")." </th><th align=left><font size=2> &nbsp; "._QXZ("ACTION")."</th></tr>\n";
		//end
		...
		while ($logs_to_print > $u) 
			...
			if ($logs_to_print > 0)
				{echo "<tr><td align=left colspan=5><font size=2> "._QXZ("Inbound Call IVR entries").": </td></tr>";}
	...
	##### vicidial agent outbound calls for this time period #####
		...
		//ishwari
		echo "<center><B>"._QXZ("RECORDINGS FOR THIS LEAD").":</B></center>\n";
		echo "<TABLE width=800 cellspacing=1 cellpadding=1>\n";
		echo "<tr><th><font size=1># </th><th align=left><font size=2> "._QXZ("LEAD")."</th><th><font size=2>"._QXZ("DATE/TIME")." </th><th align=left><font size=2>"._QXZ("SECONDS")." </th><th align=left><font size=2> &nbsp; "._QXZ("RECID")."</th><th align=center><font size=2>"._QXZ("FILENAME")."</th><th align=left><font size=2>"._QXZ("LOCATION")."</th><th align=left><font size=2>"._QXZ("TSR")."</th>$mute_column<th align=left><font size=2> </th></tr>\n";
		//end
		...
		while ($logs_to_print > $u) 
			...
			if ($SSmute_recordings > 0)
				...
				$rec_location_array = explode("/",$location);
				$location = (empty($_SERVER['HTTPS']) ? 'http' : 'https') . "://".$_SERVER['HTTP_HOST']."/RECORDINGS/".$rec_location_array[4]."/".$rec_location_array[5];
			...
			echo "<tr $bgcolor>";
			echo "<td><font size=1>$u</td>";
			echo "<td ><font size=2> $row[12] </td>";
			echo "<td ><font size=1> $row[4] </td>\n";
			echo "<td ><font size=2> $row[8] </td>\n";
			echo "<td ><font size=2> $row[0] &nbsp;</td>\n";
			echo "<td ><font size=1> $row[10] </td>\n";
			echo "<td ><font size=2> $location </td>\n";
			echo "<td ><font size=2> <A HREF=\"user_stats.php?user=$row[13]\" target=\"_blank\">$row[13]</A> </td>";
			//end
			if ($SSmute_recordings > 0)
				{
...
//16-03-21
	...
	if ($log_recording_access > 0) 
		...
		if ($admin_display > 0)
			{
			//ishwari
			echo "<center><a class='btn btn-primary btn-sm' href=\"./admin.php?ADD=720000000000000&stage=$lead_id&category=LEADS\">"._QXZ("Click here to see Lead Modify changes to this lead")."</a></center>\n";
			//end
			}
		...
		if ($qcuser_level > 0)
			{
			//ishwari
			echo "<center><a class='btn btn-primary btn-sm' href=\"qc_modify_lead.php?lead_id=$lead_id\">"._QXZ("Click here to QC Modify this lead")."</a></center>\n";
			//end
			}
		echo "\n";
		}
	...
	if ($enable_gdpr_download_deletion > 0)
		...
		if ($gdpr_display>=1) 
			{
			echo "<br>";
			echo "<center><B>"._QXZ("GDPR compliance").":</B></center>\n";
			echo "<TABLE width=750 cellspacing=2 cellpadding=5>\n";
			echo "<tr bgcolor=''>";
			echo "<td><font size=2>";
			//ishwari
			echo "<center><a class='btn btn-primary btn-sm' href=\"admin_modify_lead.php?lead_id=$lead_id&gdpr_action=download\">"._QXZ("Click here to download GDPR-formatted data for this lead")."</a></center>\n";
			//end
			echo "</font></td>";
			echo "<tr bgcolor=''>";
			echo "<td><font size=2>";
			if ($gdpr_display>=2) 
				{
				//ishwari
				echo "<center><a class='btn btn-primary btn-sm' href=\"admin_modify_lead.php?lead_id=$lead_id&gdpr_action=purge\">"._QXZ("Click here to review and purge customer data on lead")."</a></center>\n";
				//end
				}
			echo "</font></td>";
			echo "</tr>";
