...
#  
# Thanks to Joseph Johnson <freewermadmin@gmail.com>    LICENSE: AGPLv2 
# I took his report as base for this one. 
#
# Gives hourly detail of distinct agents per user group, with totals.
# For single days only
#
# CHANGES
# 170816-2012 - First build
...
if ($file_download < 1)
	...
	echo "</span>\n";
		...
		echo "<div class=\"pull-right\"><a class='btn btn-info' href=\"./AST_user_group_hourly_detail.php?DB=$DB&query_date=$query_date&start_hour=$start_hour&end_hour=$end_hour$groupQS$user_groupQS$call_statusQS&SUBMIT=$SUBMIT&search_archived_data=$search_archived_data\">v1</a>\n";
		echo '<a class="btn btn-info" href= "./admin.php?ADD=999999">'._QXZ("BACK").'</a></div> </div>
...
if ($SUBMIT && $query_date && $start_hour && $end_hour) {
	...
	while($row=mysqli_fetch_array($rslt)) {
		#echo $row["user_group"]."-->".$row["user"]."<br>";
		$temp=array();
		$temp[]=$row["user_group"];
		...
		$countByUserGroup[$row["user_group"]]+=1;
		$countUserHour[$row["hour"]]+=1;
	...
	$ASCII_header ="+----------------------+";
	$ASCII_title .="| "._QXZ("USER GROUP", 20)." |";
	$ASCII_total .="|               "._QXZ("TOTALS", 6)." |";
	...
	foreach($hour_array as $key){	
		...
		$ASCII_header.="--------------+";
		$ASCII_total.=" ".sprintf("%12s", ($countUserHour[$key]+0))." |";

		$HTML_text.="<th><font size='2'>".date("ha", strtotime("$key:00"))." to ".date("ha", strtotime("$key1:00"))."</font></th>";
		$HTML_text2.="<th><font size='2'>".($countUserHour[$key]+0)."</font></th>";
		$table_columns++;
	...
	$UG_ct=1;
	foreach($UserGroups as $Ugroup){
	#	echo "$Ugroup---->$hour--->".$maxUserByUserGroup[$Ugroup]."<br/>";
	#	echo "<br/>$Ugroup<br/>";
		foreach($hour_array as $hour){	
		...
		if ($prev_group!=$Ugroup) {
			$ASCII_text.=$ASCII_header."\n";
			if ($UG_ct<count($allData)) {$HTML_text.="<tr><td colspan='$table_columns'>&nbsp;</td></tr>";}
			$CSV_text.="\n";
		}
		$prev_group=$Ugroup;
		$UG_ct++;
...
echo "<TABLE CELLSPACING=3 CELLPADDING=3><TR ><TD class='col-md-4' VALIGN=TOP> ";
echo "<INPUT TYPE=hidden NAME=DB VALUE=\"$DB\">\n";
echo "<div class='col-lg-10 no-padding'><div class='form-group custom-input mb-10'><INPUT  TYPE=TEXT class='form-control' NAME=query_date SIZE=10 MAXLENGTH=10 VALUE=\"$query_date\"><label>"._QXZ("Date").":</label></div></div><div class='col-md-2 text-center p-18'>";
?>
...
echo "</div><div class='col-md-12'><div class='form-group custom-input input-group mb-10'><select class = 'form-control' name='start_hour'>";
for ($h=0; $h<=23; $h++) {
	if ($h==$start_hour) {$s="selected";} else {$s="";}
	echo "<option value='".substr("0$h", -2)."' $s>".substr("0$h", -2)."</option>\n";
}
echo "</select><label>"._QXZ("Start time").":</label><span class=\"input-group-addon\">:00:00</span>";
echo "</div></div>";

echo "<div class='col-md-12'><div class='form-group custom-input input-group mb-10'><select class = 'form-control' name='end_hour'>";
for ($h=0; $h<=23; $h++) {
	if ($h==$end_hour) {$s="selected";} else {$s="";}
	echo "<option value='".substr("0$h", -2)."' $s>".substr("0$h", -2)."</option>\n";
}
echo "</select><label>"._QXZ("End time").":</label><span class=\"input-group-addon\">:59:59</span>";
echo "</div></div>";

echo "<TD class='col-md-4' VALIGN=TOP >";
echo "<div class='form-group custom-input mb-10'><SELECT class='form-control' SIZE=5 NAME=group[] multiple>\n";
if  (in_array('--ALL--',$group))
...
echo "</SELECT><label> "._QXZ("Campaigns").":</label></div><BR>\n";

echo "<div class='form-group custom-input mb-10'><SELECT class='form-control' SIZE=5 NAME=user_group[] multiple>\n";
...
echo "</SELECT><label>"._QXZ("Teams/User Groups")."</label></div>\n";
...
echo "<div class='form-group custom-input mb-10'><select class='form-control' name='report_display_type'>";
if ($report_display_type) {echo "<option value='$report_display_type' selected>"._QXZ("$report_display_type")."</option>";}
echo "<option value='TEXT'>TEXT</option><option value='HTML'>HTML</option></select><label>"._QXZ("Display as:")."</label></div><BR>\n";

if ($archives_available=="Y") 
	{
	echo "<input type='checkbox' name='search_archived_data' value='checked' $search_archived_data><label>"._QXZ("&nbsp;Search archived data")."</label>\n";
	}

//echo "<BR><BR><INPUT TYPE=SUBMIT class='btn btn-bdr-sucess NAME=SUBMIT VALUE='"._QXZ("SUBMIT")."'>\n";
...
echo "<TR><TD colspan=3><div class='col-md-12 text-center'>";
echo "<INPUT class='btn btn-success' TYPE=SUBMIT NAME=SUBMIT VALUE='"._QXZ("SUBMIT")."' class='btn btn-bdr-sucess'>\n";
echo "<a class='btn btn-bdr-blue' href=\"$PHP_SELF?DB=$DB&query_date=$query_date&start_hour=$start_hour&end_hour=$end_hour$groupQS$user_groupQS$call_statusQS&file_download=1&SUBMIT=$SUBMIT&search_archived_data=$search_archived_data\">"._QXZ("DOWNLOAD")."</a>";
