...
if ($action=="display_past_month" && $month_to_display)
	...
	echo "<TR>\n";
	echo "\t<TD class='col-lg-9' ALIGN='LEFT' colspan='".(4+(count($timeoff_types)<1 ? 1 : count($timeoff_types)))."'><div class=\"form-group custom-input mb-10\">";
	echo "<select class='form-control' name='timeoff_month_to_display' onChange='LoadPTOMonth(this.value)'>\n";
	...
	echo "<select><label>"._QXZ("CURRENT MONTH DISPLAYED")."</label></div>";
	echo "</font></TD>\n";
	echo "\t<TD ALIGN='RIGHT' colspan='2'><font class='standard_bold'><font class='standard_bold'><a class='btn btn-info'  href='agent_timeoff_interface.php?action=download_csv&download_month=".$download_month."'>[DOWNLOAD]</a></font></TD>\n";
	echo "</TR>\n";


	
	echo "<TR BGCOLOR=#1A486C>\n";
	echo "<TH rowspan='2' class='text-right'><font class='standard_bold' color='#FFFFFF'>Agent</font></TH>";
	echo "<TH rowspan='2' class='text-right'><font class='standard_bold' color='#FFFFFF'>Location</font></TH>";
	echo "<TH rowspan='2' class='text-right'><font class='standard_bold' color='#FFFFFF'>Month</font></TH>";
	echo "<TH class='text-right' colspan='".(count($timeoff_types)<1 ? 1 : count($timeoff_types))."'><font class='standard_bold' color='#FFFFFF'>Planned Hours</font></TH>";
	echo "<TH rowspan='2' class='text-right'><font class='standard_bold' color='#FFFFFF'>Saved/Updated Date</font></TH>";
	echo "<TH rowspan='2' class='text-right'><font class='standard_bold' color='#FFFFFF'>Updated By</font></TH>";
	echo "<TH rowspan='2' class='text-right'><font class='standard_bold' color='#FFFFFF'>&nbsp;</font></TH>";
	echo "</TR>\n";
	if (count($timeoff_types)<1)
		...
		for ($t=0; $t<count($timeoff_types); $t++)
			{
			echo "<TH class='text-right'><font class='standard_bold' color='#FFFFFF'>".$timeoff_types[$t]."</font></TH>";
			}
		echo "</TR>\n";
		}
	...
	while($row=mysqli_fetch_array($rslt))
		...
		if ($user_row["display_month"])
			...
			echo "<TD><input type='button' class='btn alert-success' value='MODIFY' onClick=\"ToggleSpan('".$row["user"]."_UPDATE')\"></TD>\n";
			echo "</TR>\n";
			...
			for ($t=0; $t<count($timeoff_types); $t++)
				...
				echo "<tr><td align='right'><font class='standard_bold'>".$timeoff_types[$t].": </td><td align='left'><input class='form-control' type='text' value='".($user_row["$timeoff_types[$t]"]+0)."' name='".$row["user"]."_".$timeoff_types[$t]."' id='".$row["user"]."_".$timeoff_types[$t]."' size='3' maxlength='5'></td></tr>"; 
				}
			$timeoff_types_str=preg_replace('/\|$/', "", $timeoff_types_str);
			echo "<tr><td colspan='2' class='text-center'>\n";
			echo "<input type='button' class='btn btn-bdr-sucess' value='UPDATE' onClick=\"UpdateTimeInfo('$row[user]', '$timeoff_types_str', '$month_to_display')\"\n";
	...
	if (!$display_all_agents || $display_all_agents==0)
		...
		echo "<td align='left'><BR><div class=\"form-group custom-input mb-10\"><select class='form-control' name='new_agent' id='new_agent'>\n";
		echo $agent_dropdown;
		echo "</select><label>"._QXZ("Select an agent to add")."</label></div></td>";
		...
		echo "<td align='right'><font class='standard'>Hours to add:</font></td><td align='left'><input class='form-control' type='text' size='3' maxlength='5' name='new_hours' id='new_hours'></td>\n";
		...
		echo "<th><input class='btn btn-bdr-sucess' type='submit' name='submit_time_off' id='submit_time_off' value='SUBMIT'></th>\n";
		echo "\t\t</tr>\n";
		echo "\t</table>\n";
		echo "</form>";
		echo "</td>\n";
...
echo '<div id="pcont4" class="container-fluid">
		...
		<div class="col-md-12 content"><div class="block-flat">';
echo "<div class='header'><h3 class=\"pull-left\">"._QXZ("Agent time-off interface")."</h3></div>\n";
...
echo "<table width='100%' border=0 cellpadding=3 cellspacing=0>\n";
echo "<TR>\n";
echo "\t<TD class='col-lg-9' ALIGN='LEFT' colspan='".(4+(count($timeoff_types)<1 ? 1 : count($timeoff_types)))."'> ";
echo "<div class=\"form-group custom-input mb-10\"><select class='form-control' name='timeoff_month_to_display' onChange='LoadPTOMonth(this.value)'>\n";
...
while($row=mysqli_fetch_array($rslt))
	...
	if ($user_row["display_month"])
		...
		$timeoff_types_str=preg_replace('/\|$/', "", $timeoff_types_str);
		echo "<tr><td colspan='2' class='text-center'>\n";
		echo "<input type='button' class='btn btn-bdr-sucess' value='UPDATE' onClick=\"UpdateTimeInfo('$row[user]', '$timeoff_types_str', '$download_month')\"\n";
		echo "</td></tr></table></form>\n";
		echo "</span></td>\n";
		echo "<td colspan='3'></td>\n";
		echo "</tr>";
...
if (!$display_all_agents || $display_all_agents==0)
	...
	echo "<td colspan='".(6+(count($timeoff_types)<1 ? 1 : count($timeoff_types)))."'>\n";
	echo "<form action='$PHP_SELF' method='post'>\n";
	echo "<div class='col-md-3' style='padding: 5px;'><input type='hidden' class='form-control' name='new_month' id='new_month' value='$download_month'></div>\n";
	echo "\t<table width='100%'>\n";
	echo "\t\t<tr>\n";
	echo "<td align='right'><font class='standard_bold'>ADD TIMEOFF DATA:</font></td>\n";
	echo "<td align='left'><font class='standard'></font><BR><div class=\"form-group custom-input mb-10\"><select class='form-control' name='new_agent' id='new_agent'>\n";
	echo $agent_dropdown;
	echo "</select><label>"._QXZ("Select an agent to add")."</label></div></td>";
	...
	echo "<th><input type='submit' class='btn btn-bdr-sucess' name='submit_time_off' id='submit_time_off' value='SUBMIT'></th>\n";
	echo "\t\t</tr>\n";
	echo "\t</table>\n";
	echo "</form>";
	echo "</td>\n";
