...
function sec_convert($sec,$precision)
{
	...
	if ($sec < 1)
	{
		if ($precision == 'HF' || $precision == 'H')
		{
			...
			$file = basename($get_url);  
			if($file=='inbound_team_performance.php'){
				//$Ftime = round($Fhours_H_int);
				return "0";
			}
				return "0:00:00";
		}
...
//ruchita
//added new function for average columns date format
//date : 02-may-2019

function sec_convert_avg($sec,$precision)
{
	$sec = round($sec,0);
...
function horizontal_bar_chart($campaign_id,$days_graph,$title,$link,$metric,$metric_name,$more_link,$END_DATE,$download_link)
	...
	if ($max_count < 1) 
		...
		while($i < $days_graph)
			...
			echo "<tr>\n";
			echo "<td class=\"\">$Bstats_date[$i] </td>\n";
			echo "<td class=\"\"><img src=\"images/bar.png\" alt=\"\" width=\"$bar_width\" height=\"10\" style=\"vertical-align: middle; margin: 2px 2px 2px 0;\"/> $Btotal_calls[$i]</td>\n";
			echo "</tr>\n";
			$i++;
			}
		echo "</table>\n";
