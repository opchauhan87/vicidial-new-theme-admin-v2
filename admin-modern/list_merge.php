...
echo "<div class='header'><h3>"._QXZ("List Merge Tool")."</h3></div>\n";
//~ echo "<br><br><br>";
echo "<table width=$page_width cellpadding=2 cellspacing=0 class='no-border'>\n";



echo "<tr><td colspan=2 class='text-left no-border'><font face='ARIAL,HELVETICA' color=black size=3 class='no-border'> &nbsp; \n";
...
if ($submit == "submit" )
	...
	if(count($available_lists)<1) {
		...
		echo "<p><a class='btn btn-info' href='$PHP_SELF'>"._QXZ("Click here to start over.")."</a></p>\n";
		exit;
	}
	...
	echo "</form>\n";
	echo "<p><a class='btn btn-info' href='$PHP_SELF'>"._QXZ("Click here to start over.")."</a></p></center>\n";
	echo "</body>\n</html>\n";
	}
...
if (($submit != "submit" ) && ($confirm != "CONFIRM"))
	...
	if ($SSallow_manage_active_lists > 0)
		{echo "<p class=text-left>"._QXZ("This is the list merge tool. It will merge several existing lists into a single list.")."</p>";}
	else
		{echo "<p class=text-left>"._QXZ("This is the list merge tool. It will only work on inactive lists. It will merge several existing lists into a single list.")."</p>";}
	
	echo "<form action=$PHP_SELF method=POST>\n";
	echo "<center><table width=$section_width cellspacing=3>\n";

	echo "<tr><td colspan=2 class=text-left><b>"._QXZ("List Merge")."</b></td></tr>\n";

	# Available Lists
	echo "<tr ><td align=right>"._QXZ("Available Lists")."</td><td align=left>\n";
	echo "<div class='col-lg-5'><select class='form-control' size=".($num_rows<10 ? $num_rows : 10)." name=available_lists[] id='available_lists' multiple onClick='PopulateMergeMenu()'>\n";
...
echo "</select></div></td></tr>\n";
	...
	# Destination List ID
	echo "<tr ><td align=right>"._QXZ("List to merge into:")."</td><td align=left>\n";
	echo "<div class='col-lg-5'><select class='form-control' name='destination_list_id' id='destination_list_id' size='1'>\n";
	...
	#echo " <input type='checkbox' name='retain_original_list' id='retain_original_list' value='Y' ".($retain_original_list ? "checked" : "")."><font size='1'>Retain original list ID (stored in entry_list_id column)</font>";
			echo "<font>("._QXZ("Lists selected above will automatically populate here").")</font></td></tr>\n";

	# Destination List ID
	echo "<tr ><td colspan=2 class=text-left><b>"._QXZ("Alternate list information - only required if -ALTERNATE LIST- is selected above")."</b></td></tr>\n";
		echo "<tr ><td align=right>"._QXZ("Alternate list ID:")."</td><td align=left>\n";
	echo "<div class='col-lg-5'><input class='form-control' type='text' name='new_list_id' id='new_list_id' size='19' maxlength='19' value='".$new_list_id."'> </div>("._QXZ("digits only").")\n";
	echo "</td></tr>\n";
	
	echo "<tr ><td colspan=2 class=text-left><b>"._QXZ("Below fields are required ONLY if -Alternate list ID- is filled out and not an already-existing list")."</b></td></tr>\n";
echo "<tr ><td align=right>"._QXZ("List Name").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name='new_list_name' size=30 maxlength=30 value='".$new_list_name."'></div>$NWB#lists-list_name$NWE</td></tr>\n";
	echo "<tr ><td align=right>"._QXZ("List Description").": </td><td align=left><div class='col-lg-5'><input class='form-control' type=text name='new_list_description' size=30 maxlength=255 value='".$new_list_description."'></div>$NWB#lists-list_description$NWE</td></tr>\n";
	echo "<tr ><td align=right>"._QXZ("Campaign").": </td><td align=left><div class='col-lg-5'><select class='form-control' size=1 name='campaign_id'>\n";
	...
	echo "</select></div>$NWB#lists-campaign_id$NWE</td></tr>\n";
	echo "<tr ><td align=right>"._QXZ("Active").": </td><td align=left><div class='col-lg-5'><select class='form-control' size=1 name='active'><option value='Y'>"._QXZ("Y")."</option><option SELECTED>N</option></select></div>$NWB#lists-active$NWE</td></tr>\n";

	# keep debug active
	echo "<input type=hidden name=DB value='$DB'>\n";
	
	# Submit
	echo "<tr ><td colspan=2 class=text-center><input class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("submit")."'></td></tr>\n";
	echo "</table></center>\n";
	echo "</form>\n";
