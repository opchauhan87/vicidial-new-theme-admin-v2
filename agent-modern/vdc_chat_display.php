...
/***********************************************
* Simple Tree Menu- \A9 Dynamic Drive DHTML code library (www.dynamicdrive.com)
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
				...
				if (chat_creator==user)
					...
					if (!email_invite_lead_id)
						{
						document.getElementById('chat_creator_console').innerHTML="<BR/><BR/><input class='blue_btn' type='button' style=\"width:150px\" value=\"<?php echo _QXZ("INVITE"); ?>\" onClick=\"javascript:document.getElementById('email_window').style.display='block'\">\n<BR/><BR/><input class='iframe-btn-red' type='button' style=\"width:150px\" value=\"<?php echo _QXZ("END CHAT"); ?>\" onClick=\"EndChat()\">";
						}
					else 
						{
						document.getElementById('chat_creator_console').innerHTML="<BR/><BR/><input class='iframe-btn-red' type='button' style=\"width:150px\" value=\"<?php echo _QXZ("END CHAT"); ?>\" onClick=\"EndChat()\">";
						}
					}
			}
		delete xmlhttp;
		...
		xmlhttp.onreadystatechange = function() 
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
				...
				if (!start_chat_attempt_result) {
					...
					window.parent.document.callcenter_form.chat_id.value=start_chat_attempt_result;
...
?>		
	...
	<table class="no-border" width='100%' align='center' border='0' cellpadding='0' cellspacing='0'>
		<tr>
			<td colspan='3' align='center'>
				<textarea border='1' name='chat_message' id='chat_message' class='chat_window form-control' cols='100' rows='4' onkeypress="if (event.keyCode==13 && !event.shiftKey) {SendMessage(this.form.chat_id.value, this.form.user.value, this.form.chat_message.value); return false;}"></textarea>
			</td>
		</tr>
		<tr>
			<td align='left' class='chat_message d-flex align-center' valign='top'><input class='iframe-btn-blue' type='button' value="<?php echo _QXZ("SEND MESSAGE"); ?>" onClick="SendMessage(this.form.chat_id.value, this.form.user.value, this.form.chat_message.value)"><BR><BR><input type='checkbox' id='MuteCustomerChatAlert' name='MuteCustomerChatAlert'><?php echo _QXZ("Mute alert sound"); ?>
			</td>
			<td align='right' class='chat_message d-flex align-center' valign='top'><input class='iframe-btn-blue' type='button' value="<?php echo _QXZ("CLEAR"); ?>" onClick="document.getElementById('chat_message').value=''"><BR><BR>
			<?php
			if ($user_level) {
			...
			<td align='center' class='chat_message' valign='middle' ><input class='iframe-btn-red' type='button' style="width:100px" value="<?php echo _QXZ("TRANSFER"); ?>"  onClick="ToggleSpan('ChatConsoleSpan'); ToggleSpan('XferConsoleSpan'); LoadXferOptions();"><BR><BR>
			</td>
		</tr>
		<tr>
	...
	<table width='450' align='center' border='0' cellpadding='3' cellspacing='0'>
		<tr>
			...
			<td align='left' width='240'>
				<select name='ChatXferGroups' id='ChatXferGroups' onChange="document.getElementById('ChatXferAgents').selectedIndex='0'" class='chat_window form-control iframe-drop' style="width:240px">
					<option value=''><?php echo _QXZ("-- Select a group to transfer to --"); ?></option>
				</select>
			</td>
			<td align='center' valign='middle' rowspan='2' width='*'><input class='iframe-btn-blue' type='button' value="<?php echo _QXZ("TRANSFER CHAT"); ?>" onClick="SendChatXferSpan(document.getElementById('ChatXferGroups').selectedIndex, document.getElementById('ChatXferAgents').selectedIndex)"></td>
		</tr>
		<tr>
			<td align='right' class='chat_message'><?php echo _QXZ("Agents"); ?>:</td>
			<td align='left'>
				<select name='ChatXferAgents' id='ChatXferAgents' onChange="document.getElementById('ChatXferGroups').selectedIndex='0'" class='chat_window form-control iframe-drop' style="width:240px">
					<option value=''><?php echo _QXZ("-- Select an agent to transfer to --"); ?></option>
				</select>
			</td>
		</tr>
		<tr>
			<td align='center' colspan='3'><BR><input class='iframe-btn-red' type='button' style="width:100px" value="<?php echo _QXZ("CANCEL"); ?>" onClick="ToggleSpan('ChatConsoleSpan'); ToggleSpan('XferConsoleSpan');"></td>
		</tr>
	</table>
	</span>
	</td>
	<td valign='top' align='center' width='200' rowspan='2'>
	<?php

	if ($full_name) {
		echo "<span id='chat_creator_console' name='chat_creator_console'>";
		if (!$chat_id) {
			echo "<input class='iframe-btn-green' type='button' style=\"height:auto\" value=\""._QXZ("START CHAT")."\" onClick=\"StartChat()\">";

			echo "<BR/><BR/><select style='' name='startchat_group_id' id='startchat_group_id' class='chat_window form-control iframe-drop' onChange=\"document.getElementById('chat_group_id').value=this.value\">\n"; 
			echo "<option value='' selected>-- "._QXZ("CHAT GROUP")." --</option>\n";
		...
		if ($chat_creator && $chat_creator==$user) {
			...
			echo "<BR/><BR/><input class='iframe-btn-red' type='button' style=\"width:150px\" value=\""._QXZ("END CHAT")."\" onClick=\"EndChat()\">";
		}
		echo "</span>";
	}
