...
while($i < $managers_ct)
	{
	$managers[$i] = preg_replace('/[^-_0-9\p{L}]/u', '', $managers[$i]);
...
$MAIN.='<div id="pcont4" class="container-fluid">
		...
		$MAIN.= "<div class='pull-right'><a class='btn btn-info' href=\"$rerun_rpt_URL&order_by=$order_by&file_download=1\">"._QXZ("DOWNLOAD")."</a><a  class='btn btn-info'href=\"./admin.php?ADD=3111&group_id=$group[0]\">"._QXZ("MODIFY")."</a><a class='btn btn-info' href=\"./admin.php?ADD=999999\">"._QXZ("REPORTS")."</a></div></div><div class='col-md-12'><div class='content'>";
		
$MAIN.="<FORM ACTION=\"$PHP_SELF\" METHOD=GET name=vicidial_report id=vicidial_report>\n";
//~ $MAIN.="<b>"._QXZ("$report_name")."</b> $NWB#rt_monitor_log_report$NWE\n";

#$MAIN.="<FORM ACTION=\"$PHP_SELF\" METHOD=GET name=vicidial_report id=vicidial_report>\n";
$MAIN.="<TABLE CELLPADDING=3 CELLSPACING=0><TR><TD VALIGN=TOP>";
$MAIN.="<INPUT TYPE=hidden NAME=DB VALUE=\"$DB\">\n";


$MAIN.= "<div class='col-md-12'><div class='col-md-10'>";
$MAIN.= "<div class=\"form-group custom-input mb-10\"><INPUT class='form-control' TYPE=TEXT NAME=query_date MAXLENGTH=10 VALUE=\"$query_date\">";
$MAIN.= "<label>"._QXZ("Dates")."</label></div></div>";
...
$MAIN.="<BR> <div class='col-md-12' style='text-align:center;'>"._QXZ("to")."</div><BR><div class='col-md-12'><div class='col-md-10'><INPUT TYPE=TEXT class='form-control' TYPE=TEXT NAME=end_date SIZE=10 MAXLENGTH=10 VALUE=\"$end_date\"></div>";
...
$MAIN.="<TD VALIGN=TOP>";
$MAIN.="<div class=\"form-group custom-input mb-10\"><SELECT class='form-control' SIZE=5 NAME=managers[] multiple>\n";
...
$MAIN.="</SELECT><label>"._QXZ("Managers")."</label></div>\n";
$MAIN.="</TD>\n";

$MAIN.="<TD VALIGN=TOP>";
$MAIN.="<div class=\"form-group custom-input mb-10\"><SELECT class='form-control' SIZE=5 NAME=users[] multiple>\n";
...
$MAIN.="</SELECT><label>"._QXZ("Users")."</label></div>\n";
$MAIN.="</TD>\n";
...
$MAIN.="</td></tr><tr><td class=text-center colspan=5><INPUT class='btn btn-bdr-sucess' TYPE=submit NAME='SUBMIT' VALUE='"._QXZ("SUBMIT")."'></td></tr></TD></TR></TABLE>\n";
...
if ($agents_selected==0 && $campaigns_selected==0)
	...
	if (mysqli_num_rows($log_rslt)>0) {
		...
		while ($log_row=mysqli_fetch_array($log_rslt)) {
			if ($q%2==0) {$tdclass="";} else {$tdclass="";}
			$HTML_text.="<tr>\n";
			$HTML_text.="<TD><B>".$log_row["monitor_start_time"]."</B></TD>\n";
			$HTML_text.="<TD><B><a href='/vicidial/user_stats.php?user=".$log_row["manager_user"]."'>".$log_row["manager_user"]." - ".$manager_array["$log_row[manager_user]"]."</a></B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["manager_server_ip"]."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["manager_phone"]."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["manager_ip"]."</B></TD>\n";
			$HTML_text.="<TD><B><a href='/vicidial/user_stats.php?user=".$log_row["agent_user"]."'>".$log_row["agent_user"]." - ".$user_array["$log_row[agent_user]"]."</a></B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["agent_server_ip"]."</B></TD>\n";
			$HTML_text.="<TD><B>"._QXZ($log_row["agent_status"])."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["agent_session"]."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["lead_id"]."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["campaign_id"]."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["monitor_end_time"]."</B></TD>\n";
			$HTML_text.="<TD><B>".$log_row["monitor_sec"]."</B></TD>\n";
			$HTML_text.="<TD><B>"._QXZ("$log_row[monitor_type]")."</B></TD>\n";
			$HTML_text.="</TR>\n";
			$q++;
