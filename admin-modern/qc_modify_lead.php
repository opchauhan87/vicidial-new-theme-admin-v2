...
#		echo $ins_stmt."<BR>";
		...
		if (!$qc_log_id) {echo "<span class='message_block'>"._QXZ("ERROR - INSERT INTO QUALITY CONTROL QUEUE FAILED!"); exit;
			echo "</span>";}
...
function ToggleQCSpans(SpanToShow) 
	{
	var spans = document.getElementsByClassName('ToggleSpans');
	var l = spans.length;
	for (var i=0;i<l;i++) 
		{
...
echo "<div class='container-fluid' id='pcont4' style='right: 7px;'>";
echo "<div class=cl-mcont>";
   echo "<div class=row>";
   echo "<div class=col-md-12 no-padding><div class='block-flat'>";
echo "<table class=\"\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
echo "<div class='header'><h3>"._QXZ("QC $scorecard_source")." $display_ID: "._QXZ("Lead")." $lead_id - ".urldecode($lead_name)."&nbsp;&nbsp;&nbsp;<a href='$back_link'>GO BACK</a></h3></div>";
...
echo "\t<tr style='display:flex;padding:5px;'>\n";
echo "\t\t<th><input type='button' value='"._QXZ("SCORECARD")."' class='btn btn-primary btn-sm' onClick=\"ToggleQCSpans('qc_master_span')\"></th>\n";
echo "\t\t<th><input type='button' value='"._QXZ("LEAD INFO")."' class='btn btn-primary btn-sm' onClick=\"ToggleQCSpans('lead_information_span')\"></th>\n";
echo "\t\t<th><input type='button' value='"._QXZ("CALLBACK INFO")."' class='btn btn-primary btn-sm' onClick=\"ToggleQCSpans('qc_call_information')\"></th>\n";
echo "\t\t<th><input type='button' value='"._QXZ("LOG INFO")."' class='btn btn-primary btn-sm' onClick=\"ToggleQCSpans('call_logs_span')\"></th>\n";
echo "\t</tr>\n";
...
if ($end_call > 0) 
	...
	echo "<CENTER style=\"background: #fff;\" ><B><FONT FACE='Courier' COLOR=BLACK SIZE=3><a href=\"admin.php?ADD=881&campaign_id=$campaign_id\">"._QXZ("Proceed to QC CAMPAIGN")." $campaign_id "._QXZ("Queue")."</a></B><BR><BR><B><I>"._QXZ("Callback Information").":</I></B>\n";
	...
	{
	# JCJ - call span
	echo "\n\n<span class='ToggleSpans' id='qc_call_information' style='display:".$qc_call_information_visibility."'><center>\n";
	echo "<div><label>"._QXZ("PLACE A CALL TO LEAD")." :</label></div><div><iframe src=\"qc_call_client_iframe.php?phone_number=$phone_number&phone_code=$phone_code&lead_id=$lead_id&list_id=$CLlist_id&stage=DISPLAY&submit_button=YES&user=$PHP_AUTH_USER&pass=$PHP_AUTH_PW&bgcolor=E6E6E6\" style=\"background-color:#FFEEFF;\" scrolling=\"auto\" frameborder=\"1\" allowtransparency=\"true\" id=\"vcFormIFrame\" name=\"qcFormIFrame\" width=\"540\" height=\"80\" STYLE=\"z-index:18\"> </iframe></div><BR><BR>\n";
...
/* JCJ - Commented out, moved to AJAX
	...
	echo "<TABLE BGCOLOR=#".$SSstd_row4_background." WIDTH=750 align='center'><TR><TD><font class='standard_bold' style='display: flex'>";
	echo "<label>"._QXZ("Callback Details").":</font></label>";
...
<input type=hidden name=viewtime value='<?php echo $STARTtime; ?>' />
		...
		echo "<div style='text-align: left;'>"._QXZ("If you want to change this lead to a scheduled callback, first change the Disposition to CBHOLD or similar, then submit and you will be able to set the callback date and time.")."<BR>";
		echo "<BR>"._QXZ("This lead is not currently a recognized callback status, such as CALLBK or CBHOLD, or a campaign or system status where the -scheduled callback- parameter is set to -Y-.")."<BR></div>";
		}
	echo "</TD></TR></TABLE>";
	echo "</center></span>";
	################################
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	echo "\n\n<span id='lead_information_span' class='ToggleSpans' style='display:".$lead_information_span_visibility."'><CENTER>";
	...
	echo "<tr><td class='text-left' colspan=2>"._QXZ("Status").": &nbsp; &nbsp; <span id='dispo_display' class='ToggleSpans'>$dispo</span></td></tr>\n";
	echo "<tr><td class='text-left' colspan=2>$label_vendor_lead_code: <span class='ToggleSpans' id='vendor_id_display'>$vendor_id</span> &nbsp; &nbsp; "._QXZ("Lead ID").": $lead_id</td></tr>\n";
	echo "<tr><td class='text-left' colspan=2>"._QXZ("Fronter").": <span class='ToggleSpans' id='tsr_display'><A HREF=\"user_stats.php?user=$tsr\" target='_blank'>$tsr</A></span> &nbsp; &nbsp; "._QXZ("List ID").": $list_id &nbsp; &nbsp; "._QXZ("Called Count").": <span class='ToggleSpans' id='called_count_display'>$called_count</span></td></tr>\n";

	echo "<tr><td align=right>$label_title: </td><td align=left><div class='col-lg-5'><input type=text class='form-control' id=title name=title size=4 maxlength=$MAXtitle value=\"$title\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr> \n";
	echo "<td align=right>$label_first_name: </td><td><div class='col-lg-5'><input type=text class='form-control' id=first_name name=first_name size=15 maxlength=$MAXfirst_name value=\"$first_name\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div> </td></tr>\n";
	echo "<tr><td align=right>$label_middle_initial:  </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=middle_initial name=middle_initial size=4 maxlength=$MAXmiddle_initial value=\"$middle_initial\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"> </div></tr> \n";
	echo "<tr><td align=right> $label_last_name: </td><td align=left><div class='col-lg-5'><input type=text id=last_name name=last class='form-control'_name size=15 maxlength=$MAXlast_name value=\"$last_name\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div> </td></tr>\n";
	echo "<tr><td align=right>$label_address1 : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=address1 name=address1 size=40 maxlength=$MAXaddress1 value=\"$address1\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_address2 : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=address2 name=address2 size=40 maxlength=$MAXaddress2 value=\"$address2\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_address3 : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=address3 name=address3 size=40 maxlength=$MAXaddress3 value=\"$address3\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_city : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=city name=city size=40 maxlength=$MAXcity value=\"$city\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_state: </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=state name=state size=2 maxlength=$MAXstate value=\"$state\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"> </div> </tr>\n";
	echo "<tr><td align=right> $label_postal_code:</td> <td align=left><div class='col-lg-5'><input class='form-control' type=text id=postal_code name=postal_code size=10 maxlength=$MAXpostal_code value=\"$postal_code\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div> </td></tr>\n";

	echo "<tr><td align=right>$label_province : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=province name=province size=30 maxlength=$MAXprovince value=\"$province\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>"._QXZ("Country")." : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=country_code name=country_code size=3 maxlength=$MAXcountry_code value=\"$country_code\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_phone_number : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=phone_number name=phone_number size=18 maxlength=$MAXphone_number value=\"$phone_number\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_phone_code : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=phone_code name=phone_code size=10 maxlength=$MAXphone_code value=\"$phone_code\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_alt_phone : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=alt_phone name=alt_phone size=12 maxlength=$MAXalt_phone value=\"$alt_phone\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_email : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=email name=email size=40 maxlength=$MAXemail value=\"$email\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_security_phrase : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=security name=security size=40 maxlength=$MAXsecurity_phrase value=\"$security\" onBlur=\"UpdateCustomerInfo($lead_id, 'security_phrase', this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>"._QXZ("Rank")." : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=rank name=rank size=7 maxlength=5 value=\"$rank\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>"._QXZ("Owner")." : </td><td align=left><div class='col-lg-5'><input class='form-control' type=text id=owner name=owner size=22 maxlength=$MAXowner value=\"$owner\" onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\"></div></td></tr>\n";
	echo "<tr><td align=right>$label_comments : </td><td align=left><div class='col-lg-5'><TEXTAREA class='form-control' id=comments name=comments ROWS=3 COLS=65 onBlur=\"UpdateCustomerInfo($lead_id, this.name, this.value)\">$comments</TEXTAREA></div></td></tr>\n";
...
//	echo "<tr bgcolor=#B6D3FC><td align=left>Modify closer log </td><td align=left><input type=checkbox name=modify_closer_logs value=\"1\"></td></tr>\n";
	echo "<tr><td align=left>"._QXZ("Disable QC log entry")." </td><td align=left><input type=checkbox name=add_qc_record value=\"1\">("._QXZ("this feature is not active yet").")</td></tr>\n";

	echo "<tr><td style='text-align: center;' colspan=2 align=center><input class='btn btn-primary' type=submit name=submit value=\""._QXZ("SUBMIT")."\"></td></tr>\n";
...
echo "</center></span>";
	...
	# JCJ - CALL LOG span
	echo "\n\n<span id='call_logs_span' class='ToggleSpans' style='display:".$call_logs_span_visibility."'><CENTER>";

	echo "<label>"._QXZ("QUALITY CONTROL LOG RECORDS FOR THIS LEAD").":</label>\n";
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	echo "<thead><tr><th># </th><th>"._QXZ("DATE/TIME")." </th><th align=center>"._QXZ("OLD STATUS")."</th><th align=center>"._QXZ("NEW STATUS")."</th><th align=center>"._QXZ("QC USER")."</th><th align=center>"._QXZ("AGENT")."</th><th align=center>"._QXZ("CAMPAIGN")."</th><th align=center>"._QXZ("LIST")."</th><th align=center>"._QXZ("ELAPSED")."</th><th align=center>&nbsp;</th></tr></thead>\n";
	echo "$qc_agent_log";
	echo "</TABLE>\n";
	echo "<BR><BR>\n\n";

	echo "<label>"._QXZ("CALLS TO THIS LEAD").":</label>\n";
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	echo "<thead><tr><th># </th><th>"._QXZ("DATE/TIME")." </th><th align=left>"._QXZ("LENGTH")."</th><th align=left> "._QXZ("STATUS")."</th><th align=left> "._QXZ("TSR")."</th><th align=right> "._QXZ("CAMPAIGN")."</th><th align=right> "._QXZ("LIST")."</th><th align=right> "._QXZ("LEAD")."</th><th align=right> "._QXZ("HANGUP REASON")."</th><th align=right> "._QXZ("PHONE")."</th></tr></thead>\n";
	echo "$call_log";
	echo "</TABLE>\n";
	echo "<BR><BR>\n\n";

	echo "<label>"._QXZ("CLOSER RECORDS FOR THIS LEAD").":</label>\n";
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	echo "<thead><tr><th># </th><th>"._QXZ("DATE/TIME")." </th><th align=left>"._QXZ("LENGTH")."</th><th align=left> "._QXZ("STATUS")."</th><th align=left> "._QXZ("TSR")."</th><th align=right> "._QXZ("CAMPAIGN")."</th><th align=right> "._QXZ("LIST")."</th><th align=right> "._QXZ("LEAD")."</th><th align=right> "._QXZ("WAIT")."</th><th align=right> "._QXZ("HANGUP REASON")."</th></tr></thead>\n";
	echo "$closer_log";
	echo "</TABLE>\n";
	echo "<BR><BR>\n\n";


	echo "<label>"._QXZ("AGENT LOG RECORDS FOR THIS LEAD").":</label>\n";
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	echo "<thead><tr><th># </th><th>"._QXZ("DATE/TIME")." </th><th align=left>"._QXZ("CAMPAIGN")."</th><th align=left> "._QXZ("TSR")."</th><th align=left> "._QXZ("PAUSE")."</th><th align=right> "._QXZ("WAIT")."</th><th align=right> "._QXZ("TALK")."</th><th align=right> "._QXZ("DISPO")."</th><th align=right> "._QXZ("STATUS")."</th><th align=right> "._QXZ("GROUP")."</th><th align=right> "._QXZ("SUB")."</th></tr></thead>\n";
	echo "$agent_log";
	echo "</TABLE>\n";
	echo "<BR><BR>\n\n";
	...
	# JCJ - QC LOG span
	echo "\n\n<span id='qc_master_span' class='ToggleSpans' style='display:".$qc_master_span_visibility."'><CENTER>\n";
	...
	if ($qc_log_id)
		...
		if (mysqli_num_rows($checkpoint_rslt)==0)
			...
			echo "<div class='col-md-5'><label>"._QXZ("SCORECARD ID").":</label> <font size='3'>$qc_scorecard_id</font></div>";
			echo "<div class='col-md-7'><div class='col-md-3'><label>"._QXZ("RECORDING ID").": </label></div><div class='col-md-4' style='margin-bottom:5px;'><input type='text' class='form-control' name='recording_id".$qccl_id."' size='8' maxlength='10' value='$qc_recording_id'></div></div>";
			echo "</div>\n";

			echo "<tr><thead>";
			echo "<th align='left'>&nbsp;</th>";
			echo "<th align='left'>"._QXZ("Checkpoint")."</th>";
			echo "<th align='left'>"._QXZ("Fail")."?</th>";
			echo "<th>"._QXZ("Points")."</font></th>";
			echo "<th align='center'>"._QXZ("Comments")."</th>";
			...
			while ($checkpoint_row=mysqli_fetch_array($checkpoint_rslt))
				...
				echo "<td align='center' nowrap><input type='text' onBlur=\"LogQCData('".$qccl_id."')\" size=2 maxlength=6 class='form-control' name='checkpoint_points_earned".$qccl_id."' id='checkpoint_points_earned".$qccl_id."' value='$checkpoint_row[checkpoint_points_earned]'> / $checkpoint_row[checkpoint_points]</td>";
				echo "<td align='center'>";
				echo "<textarea onBlur=\"LogQCData('".$qccl_id."')\" class='form-control' name='checkpoint_comment_agent".$qccl_id."' id='checkpoint_comment_agent".$qccl_id."' rows='4' cols='40'>$checkpoint_row[checkpoint_comment_agent]</textarea>";
				echo "<BR><input type='button'  class='btn btn-primary' value='ADD REC TIMESTAMP' onClick=\"AddToComments('".$qccl_id."', 1)\">";
				echo "</td>";
...
#						echo "<option value='$preset_keyvals[0]'>".$preset_keyvals[$pi]."</option>\n";
			...
			echo "</tr>\n";
			echo "<tr>";
			echo "<td align='left'><label>"._QXZ("FINISH QC").": </label></td>";

			# echo "<tr bgcolor=#".$SSstd_row4_background."><td align=right>"._QXZ("QC Result").": </td>";
			echo "<td align=left colspan='3'><select class='form-control' size=1 name=status>\n";
...
#			echo "<td colspan='2' align='center'><input type='button' class='red_btn' style='width:100px' value='FAIL' onClick=\"FinishQCRecord('$qc_log_id', 'FAIL')\"></td>";
			echo "<td align='center'><input type='button' class='btn btn-success' style='width:100px' value='"._QXZ("FINISH")."' onClick=\"FinishQCRecord('$qc_log_id', 'FINISHED', this.form)\"></td><td align='center'><input type='button' class='btn btn-danger' style='width:100px' value='"._QXZ("RELEASE")."' onClick=\"FinishQCRecord('$qc_log_id', 'RELEASE', this.form)\"></td>";
			echo "</tr>\n";
			echo "</TABLE>\n";
			}
		echo "<BR><BR><BR>";
	...
	echo "<label>"._QXZ("ALL RECORDINGS FOR THIS LEAD").":</label>\n";
	echo "<table class=\"table_list hover centerline\" width=\"850\" cellspacing=\"0\" cellpadding=\"1\">\n";
	echo "<tr><thead><th># </th><th align=left> "._QXZ("LEAD")."</th><th>"._QXZ("DATE/TIME")." </th><th align=left>"._QXZ("SECONDS")." </th><th align=left> &nbsp; "._QXZ("RECID")."</th><th align=center>"._QXZ("FILENAME")."</th><th align=left>"._QXZ("LOCATION")."</th><th align=left>"._QXZ("TSR")."</th><th align=left>&nbsp; </th></thead></tr>\n";
