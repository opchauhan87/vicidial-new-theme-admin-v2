...
<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>
	<div id="pcont4" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3><?php echo _QXZ("VOICE LAB: Admin"); ?></h3> </div>
...
if ($NEW_VOICE_LAB > 0)
	...
	if ( (strlen($server_ip) > 6) && (strlen($session_id) > 6) && (strlen($campaign_id) > 2) )
		{
		echo ""._QXZ("TO START YOUR VOICE LAB, DIAL 9%1s ON YOUR PHONE NOW",0,'',$session_id)."<br>\n";
		...
		echo "<div align='right'  class='col-lg-5'>"._QXZ("Message Extension")."</div><div class='col-lg-2'><input class='form-control' type=text name=message></div>\n";
		echo "<div class='col-lg-2'><input  style='' class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("PLAY THIS MESSAGE")."'></div>\n";
		echo "</form><BR><BR><BR>\n";
		...
		echo "<input type=hidden name=server_ip value=$server_ip>\n";
		echo "<input type=hidden name=campaign_id value=$campaign_id>\n";
		echo "<input  style='' class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("KILL THIS VOICE LAB")."'>\n";
		echo "</form><BR><BR><BR>\n";
		}
		else
		{
	...
	if ($PLAY_MESSAGE > 0)
		...
		if ( (strlen($server_ip) > 6) && (strlen($session_id) > 6) && (strlen($campaign_id) > 2) && (strlen($message) > 0) )
			...
			echo "<input type=hidden name=campaign_id value=$campaign_id>\n";
			echo "<div align='right'  class='col-lg-5'>"._QXZ("Message Extension")."</div><div class='col-lg-2'><input class='form-control' type=text name=message></div>\n";
			echo "<div class='col-lg-2'><input class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("PLAY THIS MESSAGE")."'></div>\n";
			echo "</form><BR><BR><BR>\n";
			...
			echo "<pre style='width: 1000px;float:left'>|$stmt|</pre>\n<BR><BR><br><br>\n";
			$rslt=mysql_to_mysqli($stmt, $link);
			...
			echo "<input type=hidden name=server_ip value=$server_ip>\n";
			echo "<input type=hidden name=campaign_id value=$campaign_id>\n";
			echo "<input class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("KILL THIS VOICE LAB")."'>\n";
			echo "</form><BR><BR><BR>\n";
			}
			else
			{
		...
		echo "<label><b>"._QXZ("Start a Voice Lab Session").": 8600900</b></label>";
		echo "<input type=hidden name=NEW_VOICE_LAB value=2>\n";
		echo "<input type=hidden name=session_id value=8600900>\n";	
		echo "<table style=''><tr><td align=right>";
		echo _QXZ("Your Server").":</td><td align=left> <div class='col-lg-5'><select class='form-control' size=1 name=server_ip>$servers_list</select></div></td></tr><tr><td align=right>\n";
		echo "\n";
		echo _QXZ("Campaign").": </td><td align=left> <div class='col-lg-5'><select  class='form-control'size=1 name=campaign_id>$campaigns_list</select></td></tr>";
		echo "\n";
		echo "<td colspan=2 class=text-center><input class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("submit")."' class='btn'></td></tr>\n";
		echo "</form></table>\n";

		
		echo "<form action=$PHP_SELF method=POST>\n";
		echo "<label><b>"._QXZ("Kill a Voice Lab Session").": 8600900</b></label>";
		echo "<input type=hidden name=KILL_VOICE_LAB value=2>\n";
		echo "<input type=hidden name=session_id value=8600900>\n";
		echo "<table><tr><td align=right>"._QXZ("Your Server").": </td><td align=left> <div class='col-lg-5'><select class='form-control' size=1 name=server_ip>$servers_list</select></div></td></tr><tr><td align=right>\n";
		//~ echo "\n<td align=right>";
		echo _QXZ("Campaign").": </td><td align=left> <div class='col-lg-5'><select class='form-control' size=1 name=campaign_id>$campaigns_list</select></div></td></tr>";
		echo "\n";
		echo "<td colspan=2 class=text-center><input style='' class='btn btn-bdr-sucess' type=submit name=submit value='"._QXZ("submit")."' class='btn'></td></tr>\n";
