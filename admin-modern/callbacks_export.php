...
if ($SUBMIT) 
	...
	if (count($cb_groups)>0 && $query_date)
		...
		if (mysqli_num_rows($callback_rslt)>0) 
			{
			$rpt_output.="<label><FONT class='standard'>Total Results:</font> <font color='red' class='standard_bold'>".$callbacks_count."</FONT><a href='$PHP_SELF?query_date=$query_date&end_date=$end_date$groupsQS&file_download=1&SUBMIT=$SUBMIT'>[DOWNLOAD AS CSV FILE]</A></font></label>";	
			$rpt_output="<table class='table_list hover centerline' width='100%' cellspacing='0' cellpadding='3'>";
			$rpt_output.="<thead><tr>";
			$rpt_output.="<th nowrap>"._QXZ("Lead ID")."</FONT></th>";
			$rpt_output.="<th nowrap>"._QXZ("Phone number")."</FONT></th>";
			$rpt_output.="<th nowrap>"._QXZ("Callback status")."</FONT></th>";
			$rpt_output.="<th nowrap>"._QXZ("Created by user")."</FONT></th>";
			$rpt_output.="<th nowrap>"._QXZ("Callback entry time")."</FONT></th>";
			$rpt_output.="<th nowrap>"._QXZ("Scheduled callback time")."</FONT></th>";
			$rpt_output.="<th>"._QXZ("Notes")."</FONT></th>";
			$rpt_output.="</tr></thead>";
			...
			while($callback_row=mysqli_fetch_array($callback_rslt))
				...
				$rpt_output.="<tr>\n";
				$rpt_output.="<td nowrap>$callback_row[lead_id]</FONT></td>";
				$rpt_output.="<td nowrap>$phone_number</FONT></td>";
				$rpt_output.="<td nowrap>$callback_row[status]</FONT></td>";
				$rpt_output.="<td nowrap>$callback_row[user]</FONT></td>";
				$rpt_output.="<td nowrap>$callback_row[entry_time]</FONT></td>";
				$rpt_output.="<td nowrap>$callback_row[callback_time]</FONT></td>";
				$rpt_output.="<td>$callback_row[comments]</FONT></td>";
				$rpt_output.="</tr>";
		...
		#	$rpt_output.=$callback_stmt."*\n";
			...
			{
			$err_msg="<tr><th colspan=2><B><font color='#F00'><center style='margin-bottom: 1%;'>NO RESULTS FOUND</center></th></tr>";
			}
		}
	else 
		{
		$err_msg="<tr><th colspan=2><B><font color='#F00'><center style='margin-bottom: 1%;'>REPORT NOT RUN - SOME VALUES MISSING</center></th></tr>";
		}
	}
...
echo "<div id='pcont4' class='container-fluid'>
		<div class='cl-mcont'>
        <div class='row'>
		<div class='col-md-12 no-padding'><div class='block-flat'><div class='header'> <h3> "._QXZ('Callbacks Export')."$NWB#cb-export$NWE</h3> </div>
...
$purge_verbiage=array();
	...
	echo "<TABLE style='BORDER:0px !important;' BORDER=0 CELLPADDING=3 CELLSPACING=0 BGCOLOR=\"#".$SSframe_background."\">";
	echo "$err_msg";
	
	echo "<tr>";
echo "<TD  style='width: 50% !important;border-bottom: 0px !important;border-top: 0px !important;' VALIGN=TOP class='text-left'>";
	echo "<div class='col-md-12'><div class='col-md-11'><div class=\"form-group custom-input mb-10\"><INPUT class='form-control' TYPE=TEXT NAME=query_date SIZE=10 MAXLENGTH=10 VALUE=\"$query_date\"><label>"._QXZ("Scheduled Callback Dates")."$NWB#cb-export-cb-dates$NWE</label></div></div>";
	...
	echo "</script></div>\n";
	echo "<BR><div class='col-md-12 text-center'>"._QXZ("to")." </div><div class='col-md-12'><div class='col-md-11'><INPUT class='form-control' TYPE=TEXT NAME=end_date SIZE=10 MAXLENGTH=10 VALUE=\"$end_date\"></div>";
	echo "<script language=\"JavaScript\">\n";
	echo "var o_cal = new tcal ({\n";
	echo "	// form name\n";
	echo "	'formname': 'vicidial_report',\n";
	...
	echo "</script>\n";
	echo "</B></TD></div>";
	echo "<td  style='border-top: 0px !important;border-left: 0px !important;border-bottom: 0px !important;' align='left' width='270'>";
	...
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "$stmt\n";}
	echo "<BR><div class=\"form-group custom-input mb-10\"><select class='form-control' name='cb_groups[]' size=5 multiple>\n";
	...
	echo "</select><label>"._QXZ("Campaigns with callback")."$NWB#cb-bulk-campaigns$NWE</label></div>";
	echo "</td>";

	

	echo "<tr>";
	echo "<td  style='border-top: 0px !important;' class='text-center' colspan='2'>";
	echo "<input type='submit' class='btn btn-bdr-sucess' name='SUBMIT' value='SUBMIT'><BR/><BR/>";
	echo "</td>";
	echo "</tr>";
