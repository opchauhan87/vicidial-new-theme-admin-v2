...
$NWE = "')\"><IMG SRC=\"help.gif\" WIDTH=20 HEIGHT=20 BORDER=0 ALT=\"HELP\" ALIGN=TOP></A>";
	...
	if (!$manager_chat_id) { # DO NOT ALLOW NEW CHATS WHILE AN OLD ONE IS OPEN!!!
		...
		echo "<TD><h4>"._QXZ("CURRENT LIVE CAMPAIGNS")."</h4></TD></tr>\n";
		echo "<TR>";
		echo "<TD rowspan=3 valign='top'>";
		echo "<select class='form-control' name='available_chat_agents[]' multiple size='12' style=\"width:350px\">\n";
		...
		echo "</TD>";
		echo "<TR>";
		echo "<TD style='border-left: 1px solid #CDCDC0;'><h4>"._QXZ("CURRENT LIVE USER GROUPS")."</h4></TD>\n";
		echo "</TR>";
		echo "<TR>";
		echo "<TD valign='top' style='border-left: 1px solid #CDCDC0;'>";
		...
		echo "<TR>\n";
		echo "<TD align='left' style='text-align: left;'><h4>"._QXZ("MESSAGE").":</h4></td>";
		echo "<TD align='left'><h4>"._QXZ("SEND TO").":</h4></td>";
		echo "</TR>";
		echo "<TR>\n";
		echo "<TD align='left' style='text-align: right;'>";
		echo "<textarea class='form-control' rows='5' cols='50' name='manager_message' id='manager_message'></textarea><BR><BR>";

		echo "<input class='ace' type='checkbox' name='allow_replies' id='allow_replies' value='Y' style='margin-left: -25px; margin-top: 1px;' checked><label class='lbl'>"._QXZ("Allow agent replies")."</label>";
		echo "</TD>";
		echo "<td align='center' valign='middle'>";
		echo "<input type='submit' name='submit_chat' style='width: 150px' class='btn btn-bdr-sucess' value='"._QXZ("SELECTED AGENTS")."'><BR><BR><BR>";
		echo "<input type='submit' name='submit_chat' style='width: 150px' class='btn btn-bdr-sucess' value='"._QXZ("ALL LIVE AGENTS")."'>";
		echo "</td>";
		echo "</TR>";
		echo "</table>";
		echo "</form>";
		...
		echo "<TABLE width=750 cellspacing=1 cellpadding=1 class='hover centerline'>";
		echo "<TR>\n";
		echo "<TD align='left' colspan='3'><h4>"._QXZ("CURRENT CHAT")." #$manager_chat_id</h4></TD>\n";
		echo "<TD align='right'><a href='manager_chat_interface.php?end_all_chats=$manager_chat_id' style=\"color: #428bca;\">["._QXZ("END ALL CHATS")."]</a></td>";
		echo "</TR>";
		echo "<TR>\n";
		echo "\t<TD align='left'><h4>"._QXZ("Agent")."</h4></TD>\n";
		echo "\t<TD align='left' colspan='1'><h4>"._QXZ("Transcript")."</h4></TD>\n";
		echo "\t<TD align='left' style='text-align: left;' colspan='2'><h4>"._QXZ("Message")."</h4></TD>\n";
		echo "</TR>";
		...
		if (mysqli_num_rows($rslt)>0) 
			...
			while($row=mysqli_fetch_row($rslt)) 
				...
				if ($prev_chat_subid!=$chat_subid) 
					...
					$chat_output_header[$chat_subid]= "<TR>\n";
					...
					$chat_output_footer[$chat_subid].="\t<td width='250' align='left' valign='top'><textarea class='form-control' id='manager_chat_message_".$manager_chat_id."_".$chat_subid."' name='manager_chat_message_".$manager_chat_id."_".$chat_subid."' rows='7' cols='40' onkeypress='if (event.keyCode==13 && !event.shiftKey) {SendChatMessage($manager_chat_id, $chat_subid, \"$agent_id\"); return false;}'></textarea></td>";
					$chat_output_footer[$chat_subid].="\t<td width='75' align='center'><input type='button' style='width: 75px' class='btn btn-bdr-sucess' value='"._QXZ("SEND")."' onClick='SendChatMessage($manager_chat_id, $chat_subid, \"$agent_id\")'><BR><BR><input type='button' style='width: 75px' class='btn btn-bdr-sucess' value='"._QXZ("END")."' onClick='EndAgentChat($manager_chat_id, $chat_subid)'></td>";
