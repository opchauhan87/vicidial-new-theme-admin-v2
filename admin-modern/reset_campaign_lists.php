...
if ( ($LOGuser_level >= 9) and $LOGmodify_campaigns>0 and $LOGmodify_lists>0 and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL REPORTS/",$LOGallowed_reports)) ) )
	...
	echo '<div id="pcont4" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3> '._QXZ("Reset Lead-Called-Status for Campaigns").'</h3> </div>
	...
	echo "<center><TABLE class=\"hover centerline\" width=$section_width cellspacing=0 cellpadding=1>\n";
	echo "<thead><tr>";
	echo "<th><div class=\"col-lg-12 no-padding\"><B>"._QXZ("Select campaign")."</B></div></th>";
	echo "<th><B>"._QXZ("Reset all/active only")."</B></th>";
	echo "<th align=center><B>&nbsp;</B></th></tr></thead>\n";

		echo "<tr>";
	echo "<td>";
	echo "<div><select class=\"form-control\" name='reset_lead_called_campaigns'>\n";
	while ($campaign_row=mysqli_fetch_array($campaign_rslt)) 
		...
		echo "</select></div></td>";
	echo "<td><div><select class=\"form-control\" name='all_or_active_only'>";
	echo "<option value='Y'>"._QXZ("Active lists only")."</option>";
	echo "<option value=''>"._QXZ("All lists")."</option>";
	echo "</select></div>";
	echo "</td>";
	echo "<td><input class=\"btn btn-bdr-sucess\" type='submit' name='submit_campaign_reset' value='"._QXZ("SUBMIT")."'></td></tr>";
